...
The current milestone is chapter-scale instrumentation without a new external upload: link the selected mechanism's repair/no-change contracts to the approved CH05 `ComicPanelPlan` collection, identify the smallest panel-specific base-raster/mask evidence gap, validate runtime/bootstrap/source-control reproducibility, and prepare a controlled production demonstration that remains unexecuted until its exact external-upload scope is authorized. Preserve `AnimationShotPlan / E-Conte` as null/separate. In parallel, keep every bakeoff candidate unaccepted until authorized human review records minutes and decisions.
