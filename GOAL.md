...
The current bounded milestone is a deterministic six-panel layout-control sequence plus adversarial run-state validation. It must exercise the compiled safe zones, cast counts, causal object chain, review dependencies, immutable hashes, and retry/acceptance states without producing art or making a provider request. It exits only when malformed or partially approved inputs cannot advance, the sequence packet remains explicitly non-executable, and the next smallest acquisition/approval gap is recorded. Preserve `AnimationShotPlan / E-Conte` as null/separate and keep every provider candidate unaccepted until authorized human review records minutes and decisions.
