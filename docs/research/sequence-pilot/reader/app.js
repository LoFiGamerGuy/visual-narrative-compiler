...
(() => {
  ...
  function dirty() { state.dirty = true; notify("Draft changed. Export review JSON to keep your work; this page does not save automatically."); }
  ...
  function canvas(panel, interactive = false) {
    ...
    if (panel.candidate?.src) {
      const img = el("img"); img.src = panel.candidate.src; img.style.objectFit = "contain"; img.alt = `${panel.id} candidate ${panel.candidate.id}. Complete source image fitted without cropping. The approved beat is available in the transcript; this candidate has not been accepted.`;
  ...
  function renderCurrent() {
    ...
    $("stage-title").textContent = state.view === "reader" ? "Read the complete sequence" : `${panel.id} · ${panel.function || "Story beat"}`;
    $("stage-kicker").textContent = state.view === "reader" ? "DRAFT READER" : "PANEL REVIEW";
  ...
  function renderReader() {
    ...
    for (const panel of state.data.panels) { const f = el("figure", null, "reader-panel"); f.id = `reader-${panel.id}`; f.tabIndex = -1; f.append(canvas(panel)); if (!panel.candidate) f.append(el("figcaption", `${panel.id} · ${panel.function}. Artwork pending.`)); target.append(f); }
  }
  ...
  function exportDraft() {
    ...
    return { schema: "SequenceReviewDraft/1", title: state.data.title, plan_sha256: state.data.plan_sha256, exported_at: new Date().toISOString(), status: "draft-unaccepted", reviewer_labels_verified: false, panels: state.data.panels.map(p => ({ id: p.id, candidate_id: p.candidate?.id || null, candidate_sha256: p.candidate?.sha256 || null, lettering: structuredClone(p.lettering), protected_regions: structuredClone(p.protected_regions), observations: structuredClone(p.observations) })) };
  }
  ...
  function setView(view) {
    state.view = view === "reader" ? "reader" : "review"; document.body.classList.toggle("reading", state.view === "reader"); $("review-stage").hidden = state.view !== "review"; $("reader-stage").hidden = state.view !== "reader"; $("review-view").setAttribute("aria-pressed", String(state.view === "review")); $("reader-view").setAttribute("aria-pressed", String(state.view === "reader"));
    if (state.data) { if (state.view === "reader") renderReader(); renderCurrent(); }
  }
  ...
  function bindEvents() {
    ...
    $("export-button").addEventListener("click", () => { try { download("sequence-review-draft.json", JSON.stringify(exportDraft(), null, 2) + "\n", "application/json"); state.dirty = false; notify("Draft JSON downloaded. Reviewer claims and observations do not confer acceptance."); } catch (e) { notify(e.message, true); } });
  ...
  window.SequenceReviewApp = Object.freeze({ getState: () => ({ data: structuredClone(state.data), selected: state.selected, view: state.view, stress: state.stress, dirty: state.dirty }), exportDraft, importDraft, geometryReport, selectPanel: index => state.data && selectPanel(index), setView, get ready() { return !!state.data; } });
  bindEvents();
  (async () => {
