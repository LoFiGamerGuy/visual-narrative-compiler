...
rec={'id':aid,'tool':'image_gen.imagegen','status':'submitted','phase':'repair' if 'R1' in aid else 'primary','panels':['-'.join(aid.split('-')[:2])], 'args':args,'arguments_path':str(r/'requests'/f'{aid}.json'),'prompt_sha256':hashlib.sha256(prompt.encode()).hexdigest(),'references':[{'path':v,'sha256':hashlib.sha256((b/v).read_bytes()).hexdigest()} for v in refs],'before_art_dependency':'Complete42 script independently reviewed. Lead actually viewed selected earlier images and references; exact current custody and gear carried forward. OneP/at mostoneR1; no cap reset.','model_snapshot':None,'seed':None,'billing':None,'owner_approval':None,'invoked':True,'submitted_utc':datetime.datetime.now(datetime.timezone.utc).isoformat()}
