...
g=json.loads((b/'research/nightglass-longform/CHAPTER7-PRODUCTION-GATE.json').read_text())
...
assert panels and all(p in [f'N7-{n:02d}' for n in range(38,41)] for p in panels)
records=[json.loads(p.read_text()) for p in (r/'calls').glob('*.json') if not p.name.endswith('-tool-return.json')]
assert sum(bool(c.get('invoked')) for c in records)<g['scope_ceilings']['room-ending'],'Local finite allowance exhausted'
...
rec={'id':aid,'tool':'image_gen.imagegen','status':'prepared','phase':phase,'panels':panels,'args':args,'arguments_path':str(arguments),'prompt_sha256':hashlib.sha256(prompt.encode()).hexdigest(),'references':ref_records,'before_art_dependency':{'gate':'research/nightglass-longform/CHAPTER7-PRODUCTION-GATE.json','script_sha256':g['script_sha256'],'preceding_zip_sha256':g['preceding_zip_sha256'],'actual_reference_review':'Worker actually viewed each reference before this call; source roles/current state recorded in prompt.'},'model_snapshot':None,'seed':None,'billing':None,'owner_approval':None,'invoked':False,'prepared_utc':datetime.datetime.now(datetime.timezone.utc).isoformat()}
p.write_text(json.dumps(rec,indent=2)+'\n');print(aid,'prepared exact provenance; no invocation yet')
