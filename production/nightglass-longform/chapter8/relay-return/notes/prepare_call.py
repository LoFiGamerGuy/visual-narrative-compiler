...
r=b/'production/nightglass-longform/chapter8/relay-return'
...
dependencies=json.loads((r/'notes/DEPENDENCIES.json').read_text())
assert dependencies['art_released'] is True, 'Hold: actual washhouse04/Edda05/load10/authorization14 dependencies not released'
if dependencies['checkpoint']['parent_checkpoint_received']:
 assert hashlib.sha256((b/dependencies['checkpoint']['receipt_path']).read_bytes()).hexdigest()==dependencies['checkpoint']['receipt_sha256'], 'Checkpoint receipt changed'
...
assert panels and all(p in [f'N8-{n:02d}' for n in range(15,33)] for p in panels)
...
assert dependencies['checkpoint']['parent_checkpoint_received'] or sum(bool(c.get('invoked')) for c in records)<dependencies['checkpoint']['local_calls_allowed_before_parent_checkpoint'], 'Pause at cumulative320: parent recorded checkpoint required'
...
rec={'id':aid,'tool':'image_gen.imagegen','status':'submitted','phase':phase,'panels':panels,'args':args,'arguments_path':str(arguments),'prompt_sha256':hashlib.sha256(prompt.encode()).hexdigest(),'references':ref_records,'before_art_dependency':{'gate':'research/nightglass-longform/CHAPTER8-PRODUCTION-GATE.json','script_sha256':g['script_sha256'],'gate_sha256':hashlib.sha256((b/'research/nightglass-longform/CHAPTER8-PRODUCTION-GATE.json').read_bytes()).hexdigest(),'preceding_zip_sha256':g['preceding_zip_sha256'],'actual_reference_review':'Editorial worker actually viewed each reference before this call; source roles/current state recorded in prompt.'},'model_snapshot':None,'seed':None,'billing':None,'owner_approval':None,'invoked':True,'submitted_utc':datetime.datetime.now(datetime.timezone.utc).isoformat()}
