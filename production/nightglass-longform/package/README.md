...
Build each milestone only after the integrator marks its required chapters complete in the reviewed reader snapshot: `chapter1` requires Chapter 1, `threechapters` requires Chapters 1–3, `fourchapters` requires Chapters 1–4, `fivechapters` requires Chapters 1–5, and `sixchapters` requires Chapters 1–6. Every required chapter must exist, be reviewed complete, and have all its artwork available. The corresponding starters are labeled “Four-chapter development delivery”, “Five-chapter development delivery” and “Six-chapter development delivery”. Use a new version for each delivery; existing versions are never overwritten. `trial` explicitly labels an incomplete snapshot and is not needed for the planned checkpoints. The default ignored destination is `production/nightglass-longform/package/output/`.
