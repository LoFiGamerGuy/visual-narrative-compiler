...
python production/nightglass-longform/package/pack.py build --version Nightglass-SixChapters-v1 --milestone sixchapters
python production/nightglass-longform/package/pack.py build --version Nightglass-SevenChapters-v1 --milestone sevenchapters
```

Build each milestone only after the integrator marks its required chapters complete in the reviewed reader snapshot: `chapter1` requires Chapter 1, `threechapters` requires Chapters 1–3, `fourchapters` requires Chapters 1–4, `fivechapters` requires Chapters 1–5, `sixchapters` requires Chapters 1–6, and `sevenchapters` requires Chapters 1–7. Every required chapter must exist, be reviewed complete, and have all its artwork available. The corresponding starters are labeled “Four-chapter development delivery”, “Five-chapter development delivery”, “Six-chapter development delivery” and “Seven-chapter development delivery”. Use a new version for each delivery; existing versions are never overwritten. `trial` explicitly labels an incomplete snapshot and is not needed for the planned checkpoints. The default ignored destination is `production/nightglass-longform/package/output/`.
