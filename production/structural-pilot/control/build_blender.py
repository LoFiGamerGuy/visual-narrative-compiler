...
if any(OUT.glob('*.png')):
 raise SystemExit('Refusing to overwrite preserved rendered guides. Copy scene.json into a NEW version directory, then render that version.')
...
def architecture(panel):
 ...
 cube('shutter.south_partition_return',(shutter_x,-1.50,1.7),(.35,.80,3.4),'stone','foreground')
 cube('shutter.north_partition_return',(shutter_x,1.78,1.7),(.35,.24,3.4),'stone','foreground')
...
def props(panel):
 ...
 for i,points in enumerate(panel.get('fracture_splinter_lines',[])):
  line('staff.contact_splinter.'+str(i),points,.012,'ivory','props')
 ...
 if 'air_brake_arrival' in [fx['id'] for fx in panel['effects']]:
  for i in range(3):
   x=3.52+i*.12
   dz=panel.get('air_brake_height_offset',0)
   line('effect.braking_tick.'+str(i),[[x,-.70,3.43+dz],[x+.18,-.7,3.05+dz]],.023,'cyan','effects')
...
for panel_id,panel in spec['panels'].items():
 ...
 for pass_name in args.passes:
  ...
  scene.render.film_transparent=pass_name not in ['full','background']
