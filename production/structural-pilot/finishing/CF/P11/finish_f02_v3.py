...
import json,hashlib
ROOT=Path(__file__).resolve().parents[5];P=Path('production/structural-pilot/finishing/CF/P11/F02-v3');old=P.parent/'F02'
...
for y in range(h):
 for x in range(w):
  ...
  if gy<504:
   ...
   left_rgb=tuple(c*.73 for c in base);band_rgb=tuple(base[k]*.55+[12,19,22][k] for k in range(3))
   if gx<left-.8:rgb=left_rgb
   elif gx<left+.8:rgb=blend(left_rgb,band_rgb,(gx-left+.8)/1.6)
   elif gx<boundary-.8:rgb=band_rgb
   elif gx<boundary+.8:rgb=blend(band_rgb,base,(gx-boundary+.8)/1.6)
   else:rgb=base
   overhead=.83+.17*min(1,max(0,(gy-504)/26))
   rgb=tuple(round(c*overhead) for c in rgb);surface='source stone texture with left plane,14px diagonal shadow band and right lit plane'
  else:
   sx,sy=gx,gy+70;rgb=tuple(round(c*.94) for c in ip[sx,sy]);surface='cream right jamb, same vertical plane'
  pp[x,y]=rgb+(255,);records.append([gx,gy,sx,sy,surface])
patch.save(ROOT/P/'arrow-repair-patch.png')
recipe={'method':'Conventional explicit source-pixel clone by four observed architectural surfaces; no learned repair','source':s['layers'][0]['path'],'source_sha256':s['layers'][0]['sha256'],'mask':str(P.parent/'F01/arrow-removal-mask.png'),'mask_sha256':hashlib.sha256((ROOT/P.parent/'F01/arrow-removal-mask.png').read_bytes()).hexdigest(),'surface_rules':['gy<504: source(gx-50,gy-3) follows unchanged cable slope','gx<858 below cable: source(gx,gy+110) cream wall','858<=gx<907: source stone atx861..895,y640..676; right shadow boundaryx=1072-y/3,leftboundary14px left; leftplane .73gain, shadow .55gain+[12,19,22], rightplane1gain,1.6px edgeblend; overhead .83..1 fromy504..530','gx>=907: source(gx,gy+70)*.94 rightjamb'],'pixel_map':records,'learned_model':None,'generation_calls':0}
(ROOT/P/'repair-recipe.json').write_text(json.dumps(recipe,indent=2)+'\n');s['id']='CF-P11-F02-v3';s['output_svg']=str(P/'P11-F02-v3.svg');s['description']='Same declared correction round final: explicit source texture cloning along cable and observed stone planes, with a drawn diagonal cast-shadow rule, replaces both row steps and harmonic blur. Thin boot-origin motion and closed cap remain. Actual-art review pending.';s['layers'][1]['path']=str(P/'arrow-repair-patch.png');s['edit_intent']['repair_final']='surface-aware source clone with exact editable recipe'
