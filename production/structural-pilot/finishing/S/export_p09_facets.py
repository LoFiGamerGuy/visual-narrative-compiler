...
ROOT=Path(__file__).resolve().parents[4];OUT=ROOT/'production/structural-pilot/finishing/S/P09/F01'
bpy.context.window.scene=bpy.data.scenes['P09'];scene=bpy.context.scene;bpy.context.view_layer.update()
...
(OUT/'kite-facets.json').write_text(json.dumps({'source_blend':'production/structural-pilot/control/v8/scene-all.blend','panel':'P09','faces':faces,'svg':svg},indent=2)+'\n')
