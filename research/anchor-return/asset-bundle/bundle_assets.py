#!/usr/bin/env python3
"""AR-20260909-01 portable share archive: frozen inputs, safe restore, offline rebuild."""
import argparse
import hashlib
import json
import os
...
ROOT = HERE.parents[2]
PREFIXES = ('production/anchor-return/', 'research/anchor-return/', 'docs/anchor-return/', 'docs/texture-refinement-kit/skill/texture-refinement/')
...
COMPOSITION_IDS = {f'{s}{i:02}' for s in ('NG','BP','ST') for i in range(1,9)} | {'SB01','IO01','FL01','RC01','CE01','HC01'}
SUPPORT_IDS = {'NG-SHEET','BP-SHEET','ST-SHEET'}
EXPECTED_IDS = COMPOSITION_IDS | SUPPORT_IDS
BASE_BUDGET = {'primary':30,'support':3,'structural_repairs':6,'texture_passes':30,'maximum_native_attempts':69,'transport_retries':2,'maximum_total':71}
PRODUCTION_BUDGET = {'primary':30,'support':3,'repair':6,'texture':30,'total_native':69,'no_art_retries':2,'total_invocations':71,'direct_paid_spend_usd':0,'max_repair_per_image':1,'max_texture_per_image':1}
...
def validate_entries(manifest):
    if manifest.get('schema') != 'AnchorReturnPortable/1' or manifest.get('experiment_id') != 'AR-20260909-01':
...
def resolve_budget(root, plan, attempts):
    base=plan['budget']
    if base != PRODUCTION_BUDGET or any(type(v) is not int for v in base.values()):raise ValueError('Only exact frozen 30P/3support/6R1/30F1, 69-native/71-invocation budget is authorized')
    if plan.get('scope_amendment') or list((root/'production/anchor-return').glob('*amendment*.json')):raise ValueError('No scope or budget amendment authorized')
...
def transport_ledger(root, attempts, maximum, strict=False):
    ...
    prod=root/'production/anchor-return'; failures={}; events={}; ledger=[]; tool_receipts=[]
    def identity(record):
    ...
    for p in sorted((prod/'transport-failures').glob('*.json')):
        ...
        if record.get('schema')=='AnchorReturnToolFailure/1':
            tool_receipts.append({'path':rel,'sha256':hashed(p)[1],'record':record});continue
        if record.get('schema') not in (None,'AnchorReturnTransportFailure/1'):raise ValueError('Unknown transport failure schema')
        ...
        ledger.append({'path':rel,'sha256':hashed(p)[1],'attempt_id':record['attempt_id'],'started':key[1],'state':'failed-no-artwork-returned','schema_validation':'AnchorReturnTransportFailure/1' if record.get('schema') else 'explicit field contract; source has no schema tag'})
...
def plan_versions(root):
    ...
    prod=root/'production/anchor-return';versions={}
    ...
    for path in paths:
        ...
        if record.get('schema')!='AnchorReturnPlan/1' or record.get('experiment_id')!='AR-20260909-01' or (len(record.get('entries',[])),frozenset(e.get('id') for e in record.get('entries',[]))) != (len(COMPOSITION_IDS),frozenset(COMPOSITION_IDS)) or {e.get('id') for e in record.get('support_entries',[])}!=SUPPORT_IDS:raise ValueError('Historical plan schema/experiment/IDs differ')
...
def prior_source_records(root, plan, binding):
    ...
    declared=plan.get('prior_choices') or {'path':'production/anchor-return/previous/ce-selection.json','sha256':hashed(root/'production/anchor-return/previous/ce-selection.json')[1]}
    if isinstance(declared,list):
        if len(declared)!=1:raise ValueError('Expected one exact historical choice export')
        declared=declared[0]
    if not declared.get('path','').startswith('production/anchor-return/previous/') or not declared.get('sha256'):raise ValueError('Exact prior choice export must be locally hash-bound')
    binding(declared['path'],declared['sha256'])
    rows=[]
    for path in sorted((root/'production/anchor-return/previous').glob('*.json')):
        rel=path.relative_to(root).as_posix();digest=hashed(path)[1];binding(rel,digest);record=json.loads(path.read_text())
        rows.append({'path':rel,'sha256':digest,'schema':record.get('schema') if isinstance(record,dict) else None,'experiment_id':record.get('experiment_id') if isinstance(record,dict) else None,'historical_plan_sha256':record.get('plan_sha256') if isinstance(record,dict) else None,'historical_dataset_sha256':record.get('dataset_sha256') if isinstance(record,dict) else None,'validation':'Exact copied file bytes; embedded prior experiment hashes are historical provenance, not AR job/call plan references.'})
    return rows
...
def source_bindings(root, strict=False, expected_art=None):
    ...
    root = root.resolve(); prod=root/'production/anchor-return'
    plan=json.loads((prod/'plan.json').read_text())
    if plan.get('schema')!='AnchorReturnPlan/1' or plan.get('experiment_id')!='AR-20260909-01' or len(plan['entries'])!=len(COMPOSITION_IDS) or {e['id'] for e in plan['entries']}!=COMPOSITION_IDS or {e['id'] for e in plan.get('support_entries',[])}!=SUPPORT_IDS:
        raise ValueError('Expected exact authorized thirty-composition/three-support AR plan')
    ...
    def walk(value, job_origin=None):
        if isinstance(value,list):
            ...
            if value.get('evidence_path') and not value.get('evidence_sha256'):
                ...
                if not path.startswith(('research/anchor-return/','production/anchor-return/')):raise ValueError('Evidence outside current experiment')
    ...
    for e in rows:
        ...
        if e['path']!='production/anchor-return/candidates/'+e['attempt_id']+'.png': raise ValueError('Unexpected native path')
        callpath=e.get('call_path')
        if callpath!='production/anchor-return/calls/'+e['attempt_id']+'.json': raise ValueError('Missing exact call record')
        ...
        for ref in call.get('references',[]):
            if not ref['path'].startswith('production/anchor-return/'):raise ValueError('Generation references must be task-owned production assets')
    ...
    if controls_path.exists():
        ...
        expected_controls={f'{s}{i:02}-CONTROL' for s in ('NG','BP','ST') for i in range(1,9)}
        actual_controls=[c['id'] if c['id'].endswith('-CONTROL') else c['id']+'-CONTROL' for c in control_doc['controls']]
        if len(actual_controls)!=len(set(actual_controls)) or set(actual_controls)-expected_controls:raise ValueError('Unknown or duplicate editable controls')
        if strict and set(actual_controls)!=expected_controls:raise ValueError('Final package requires all twenty-four editable controls')
        for c in control_doc['controls']:
            for path_key,hash_key in [('png_path','png_sha256'),('svg_path','svg_sha256')]:binding(c[path_key],c[hash_key])
    elif strict:raise ValueError('Final editable control manifest missing')
    ...
    if (prod/'input-validation-failures').exists():raise ValueError('No changed-input retry exception in AR')
    ...
    return {'pass':True,'complete':not missing and len(selections)==len(EXPECTED_IDS),'registered_attempts':len(rows),'phase_counts':phases,'selected_count':len(selections),'missing_primaries':missing,'checked_bindings':checked,'plan_sha256':plan_sha,'preserved_plan_versions':versions,'prior_source_records':prior_records,'primary_compositions':sum(x['id'] in COMPOSITION_IDS and x['attempt_id'].endswith('-P') for x in rows),'primary_support_sheets':sum(x['id'] in SUPPORT_IDS and x['attempt_id'].endswith('-P') for x in rows),'call_plan_bindings':call_plans,'budget':budget_record,'evidence_without_call_hash':list(evidence_observations.values()),'archived_unrun_job_prompts':archived_jobs,'invocation_ledger':invocations}

def gather(root, expected_art=None, strict=False):
    root=root.resolve(); binding_report=source_bindings(root,strict,expected_art)
    helper=root/'research/anchor-return/asset-bundle'; sources={}; excluded=[]
    ...
    manifest={'schema':'AnchorReturnPortable/1','experiment_id':'AR-20260909-01','files':entries,'total_bytes':sum(e['bytes'] for e in entries),'source_bindings':binding_report,'explicit_external_receipt_or_launcher_mappings':excluded,'exclusions':'Scratch/runtime/caches/venvs/browser profiles/local directories; no old galleries or original tool-return locations. Complete new namespaces plus AGENTS and textual texture skill/guide only.'}
...
def write_archive(path, manifest, sources):
    ...
    return dict(schema='AnchorReturnPortableReceipt/1',archive_bytes=size,archive_sha256=sha,manifest_sha256=hashlib.sha256(encoded(manifest)).hexdigest(),file_count=len(manifest['files']))
...
def local_links(root):
    root=root.resolve(); docs=root/'docs/anchor-return'; count=0; external=set()
...
def rebuild_check(target):
    ...
    docs=target/'docs/anchor-return'; names=['data.json','data.js']
    before={n:hashed(docs/n) for n in names}
    binding=source_bindings(target,strict=True,expected_art=len(json.loads((target/'production/anchor-return/candidates.json').read_text())['candidates']))
    run=subprocess.run([sys.executable,str(target/'research/anchor-return/reader/build_reader.py'),'--require-complete'],cwd=target,env={**os.environ,'PYTHONDONTWRITEBYTECODE':'1'},text=True,capture_output=True,check=True)
...
def create(root,out,count):
    ...
    allowed=root/'research/anchor-return/asset-bundle/local'
    ...
    archive=out/'anchor-return-portable.zip'
...
def self_test():
    ...
    for name,body in [('a.txt',b'AAA'),('b.txt',b'BBB')]:
        p=tmp/name;p.write_bytes(body);sources['production/anchor-return/'+name]=p
    entries=[dict(path=name,bytes=hashed(p)[0],sha256=hashed(p)[1]) for name,p in sorted(sources.items())]
    manifest=dict(schema='AnchorReturnPortable/1',experiment_id='AR-20260909-01',files=entries,total_bytes=6);archive=tmp/'good.zip';receipt=write_archive(archive,manifest,sources)
    checks=[]
    def rejects(label,fn,types=(ValueError,FileExistsError)):
        try:fn()
        except types:checks.append(label)
        else:raise AssertionError(label+' accepted')
    target=tmp/'restore';clash=target/'production/anchor-return/b.txt';clash.parent.mkdir(parents=True);clash.write_bytes(b'BAD')
    ...
    for bad in ['../escape','production/anchor-return/../escape','other/a.png','production/anchor-return/AUX.txt','production\\anchor-return\\a','/absolute']:
    ...
    for name in ('AGENTS.md','START_HERE.cmd','docs/anchor-return/index.html'):
        safe_path(name)
    checks.append('AR allowed namespace and root instruction paths')
    for bad in ['production/anchor-returnx/a.txt','research/world-combat/a.txt','docs/anchor-return/name.','docs/anchor-return/x:y']:
    ...
    for label,names,mode in [('duplicate archive',[entries[0]['path'],entries[0]['path']],None),('symlink archive',[e['path'] for e in entries],stat.S_IFLNK|0o777),('unexpected member',[e['path'] for e in entries]+['production/anchor-return/c.txt'],None)]:
    ...
    web=tmp/'web';docs=web/'docs/anchor-return';docs.mkdir(parents=True)
    ...
    (docs/'data.json').write_text(json.dumps({'src':'sample%20image.png?native=1#crop','record_path':'docs/anchor-return/data.json?record=1#x','tool_output_path':'docs/anchor-return/data.json','prompt_path':'docs/anchor-return/data.json','source_path':'/historical/unneeded.png','story_path':'arrival'}))
    ...
    for bad in ['production/anchor-return/.git/config','research/anchor-return/asset-bundle/local/a.zip']:
        rejects('excluded member '+bad,lambda bad=bad:safe_path(bad))
    budgetroot=tmp/'budget';bp=budgetroot/'production/anchor-return';bp.mkdir(parents=True)
    base={'budget':dict(PRODUCTION_BUDGET)}
    (bp/'plan.json').write_bytes(encoded(base));records={}
    assert resolve_budget(budgetroot,base,records)['effective_budget']==BASE_BUDGET;checks.append('fixed30 compositions3support6R1/30F1,69-native71-invocation budget')
    ...
    tr=tmp/'transport';tp=tr/'production/anchor-return';(tp/'transport-failures').mkdir(parents=True);(tp/'calls').mkdir()
    failrel='production/anchor-return/transport-failures/NG01-F1-service-call-01.json';fp=tr/failrel
    reference={'path':'production/anchor-return/candidates/NG01-P.png','sha256':'a'*64}
    failed={'id':'NG01','attempt_id':'NG01-F1','phase':'texture','category':'vista','retry_of':'NG01-P','prompt_path':'production/anchor-return/prompts/NG01-F1.txt','prompt_sha256':'b'*64,'plan_sha256':'c'*64,'references':[reference],'referenced_image_paths':['/original/'+reference['path']],'started':{'current_time':'2026-09-08 01:00:00 UTC'},'finished':{'current_time':'2026-09-08 01:01:00 UTC'},'status':'failed-no-artwork-returned','error':'HTTP503','returned_artwork':None}
    ...
    tool={'schema':'AnchorReturnToolFailure/1','functions_cell_id':'fixture','failure_record_path':failrel,'failure_record_sha256':hashed(fp)[1],'outcome':{'id':'NG01-F1','status':'error','error':'HTTP503'}}
    ...
    result=dict(schema='AnchorReturnBundleSecurityTests/1',status='pass',checks=checks,fixture_directory=str(tmp.relative_to(ROOT)),production_archive_created=False,helper_sha256=hashed(HERE/'bundle_assets.py')[1])
