...
(P/'candidates.json').write_text(json.dumps(dict(schema='AnchorReturnCandidates/1',experiment_id='AR-20260909-01',candidates=rows),indent=2)+'\n')
sel=P/'selected.json';s=json.loads(sel.read_text()) if sel.exists() else dict(schema='AnchorReturnDisplaySelection/1',experiment_id='AR-20260909-01',selected={},owner_approval=None)
for c in rows:
