#!/usr/bin/env python3
"""Read-only CD baseline audit; write one fresh receipt in asset-bundle/local only."""
import argparse
...
ROOT=HERE.parents[2]
BASELINE=ROOT/'research/combat-depth/protected-initial.json'
ACTIVE_REF='refs/heads/autonomous/nightglass-combat-depth-20260908-2310'
REMOTE_REF='refs/remotes/origin/autonomous/nightglass-combat-depth-20260908-2310'
...
def audit():
    ...
    if len(baseline['worktrees'])!=20 or len(baseline['source_files'])!=65:raise ValueError('Unexpected baseline coverage')
    paths=[x['path'] for x in baseline['worktrees']]
    if len(set(paths))!=20 or str(ROOT) in paths:raise ValueError('Protected list must contain20 unique prior worktrees and exclude active root')
    if baseline.get('schema')!='CombatDepthProtectedBaseline/1' or baseline.get('status_args')!=STATUS_ARGS:raise ValueError('Baseline schema or recorded status command differs')
    ...
    return {'schema':'CombatDepthPreservationAudit/1','experiment_id':'CD-20260908-01','started_utc':started,'finished_utc':datetime.now(timezone.utc).isoformat(),
        ...
        'limits':['Status digest includes ignored paths exactly as Git reports them; it does not byte-hash ignored/cache file contents or every tracked/untracked file.',
                  'Only 65 saved original source files have content SHA verification. Existing modified files can retain the same Git status while their bytes change.',
