...
(P/'candidates.json').write_text(json.dumps(dict(schema='CombatDepthCandidates/1',experiment_id='CD-20260908-01',candidates=rows),indent=2)+'\n')
sel=P/'selected.json';s=json.loads(sel.read_text()) if sel.exists() else dict(schema='CombatDepthDisplaySelection/1',experiment_id='CD-20260908-01',selected={},owner_approval=None)
for c in rows:
