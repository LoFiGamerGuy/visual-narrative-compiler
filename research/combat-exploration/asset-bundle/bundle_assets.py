#!/usr/bin/env python3
"""CE-20260908-01 portable share archive: frozen inputs, safe restore, offline rebuild."""
import argparse
import hashlib
import json
import os
...
ROOT = HERE.parents[2]
PREFIXES = ('production/combat-exploration/', 'research/combat-exploration/', 'docs/combat-exploration/')
...
def gather(root, expected_art):
    if type(expected_art) is not int or not 18 <= expected_art <= 24:
        raise ValueError('Expected final native generation count must be18–24')
    records = []
    for filename in ('candidates.json',):
        records += json.loads((root/'production/combat-exploration'/filename).read_text())['candidates']
    ...
    if sum(e['attempt_id'].endswith('-P') for e in records) != 18:
        raise ValueError('Expected exactly18 primary generations')
    refs = json.loads((root/'production/combat-exploration/references.json').read_text())
    if len(refs) != 9:
    ...
    for prefix in PREFIXES:
        ...
        for directory, folders, files in os.walk(base, followlinks=False):
            ...
            for name in sorted(files):
                ...
                if source.is_relative_to(root/'research/combat-exploration/asset-bundle'):
                    continue
    ...
    for name in ('bundle_assets.py', 'README.md', 'coverage-preflight.json', '.gitignore'):
        sources['research/combat-exploration/asset-bundle/'+name] = HERE/name
    sources['START_HERE.cmd'] = HERE/'START_HERE.cmd'
    entries = []
    for name,path in sorted(sources.items()):
        size,sha = hashed(path)
        entries.append(dict(path=name, bytes=size, sha256=sha))
    manifest = dict(schema='CombatExplorationPortable/1', experiment_id='CE-20260908-01', generated_native_count=len(records), primary_count=18, copied_reference_count=9, files=entries, total_bytes=sum(e['bytes'] for e in entries), exclusions='Runtime, scratch, caches, venv, prior namespaces and original tool-return locations. Current copied native assets and gallery copies retain exact paths.')
...
def write_archive(path, manifest, sources):
    ...
    size,sha = hashed(path)
    return dict(schema='CombatExplorationPortableReceipt/1',archive_bytes=size,archive_sha256=sha,manifest_sha256=hashlib.sha256(encoded(manifest)).hexdigest(),file_count=len(manifest['files']))
...
def rebuild_check(target):
    ...
    docs = target/'docs/combat-exploration'
    names = ['data.json','data.js']
    before = {name:hashed(docs/name) for name in names}
    run = subprocess.run([sys.executable,str(target/'research/combat-exploration/reader/build_reader.py'),'--require-complete'],cwd=target,env={**os.environ,'PYTHONDONTWRITEBYTECODE':'1'},text=True,capture_output=True,check=True)
...
def create(root, out, count):
    ...
    archive = out/'combat-exploration-portable.zip'
...
def self_test():
    ...
    for name,body in [('a.txt',b'AAA'),('b.txt',b'BBB')]:
        p=tmp/name;p.write_bytes(body);sources['production/combat-exploration/'+name]=p
    ...
    target=tmp/'restore';clash=target/'production/combat-exploration/b.txt';clash.parent.mkdir(parents=True);clash.write_bytes(b'BAD')
    ...
    for bad in ['../escape','production/combat-exploration/../escape','other/a.png','production/combat-exploration/AUX.txt','production\\combat-exploration\\a','/absolute']:
    ...
    for label,names,mode in [('duplicate archive',[entries[0]['path'],entries[0]['path']],None),('symlink archive',[e['path'] for e in entries],stat.S_IFLNK|0o777),('unexpected member',[e['path'] for e in entries]+['production/combat-exploration/c.txt'],None)]:
