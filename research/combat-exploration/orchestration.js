...
async function run(jobs) {
 const root=load("combatRoot"),q=s=>"'"+s.replace(/'/g,"'\\''")+"'";
 ...
 async function save(data,initial=false) {const code="from pathlib import Path;p=Path("+JSON.stringify(root+"/production/combat-exploration/calls/"+j.attempt_id+".json")+");"+(initial?"assert not p.exists();":"")+"p.write_text("+JSON.stringify(JSON.stringify(data,null,2)+"\n")+")";
 const r=await tools.exec_command({cmd:"python3 -c "+q(code),max_output_tokens:300});if(r.exit_code!==0)throw Error(r.output);}
 await save(rec,true);
 const result=await tools.image_gen__imagegen({prompt,referenced_image_paths:paths});generatedImage(result);
 const receipt={...rec,finished:await tools.clock__curr_time({}),output_hint:result.output_hint};store("ceResult:"+j.attempt_id,receipt);await save(receipt);
