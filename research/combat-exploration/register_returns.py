...
P=ROOT/'production/combat-exploration'
...
(P/'candidates.json').write_text(json.dumps({'schema':'CombatExplorationCandidates/1','experiment_id':'CE-20260908-01','candidates':rows},indent=2)+'\n')
selection_path=P/'selected.json'
selected=json.loads(selection_path.read_text()) if selection_path.exists() else {'schema':'CombatExplorationSelection/1','experiment_id':'CE-20260908-01','selected':{},'owner_approval':None}
for c in rows:
