#!/usr/bin/env python3
"""EN-20260909-01 portable share archive: frozen inputs, safe restore, offline rebuild."""
import argparse
import hashlib
import json
import os
...
ROOT = HERE.parents[2]
PREFIXES = ('production/encounter-lab/', 'research/encounter-lab/', 'docs/encounter-lab/', 'docs/texture-refinement-kit/skill/texture-refinement/')
...
EXPECTED_IDS = {f'{s}{i:02}' for s in ('NG','BP','IO','ST','SB','X') for i in range(1,5)}
BASE_BUDGET = {'primary':24,'structural_repairs':6,'texture_passes':24,'maximum_native_attempts':54,'transport_retries':2,'maximum_total':56}
PRODUCTION_BUDGET = {'primary':24,'repair':6,'texture':24,'total_native':54,'no_art_retries':2,'total_invocations':56,'direct_paid_spend_usd':0,'max_repair_per_image':1,'max_texture_per_image':1}
...
def validate_entries(manifest):
    if manifest.get('schema') != 'EncounterLabPortable/1' or manifest.get('experiment_id') != 'EN-20260909-01':
...
def resolve_budget(root, plan, attempts):
    base=plan['budget']
    if base != PRODUCTION_BUDGET or any(type(v) is not int for v in base.values()):raise ValueError('Only exact frozen 24P/6R1/24F1, 54-native/56-invocation budget is authorized')
    if plan.get('scope_amendment') or list((root/'production/encounter-lab').glob('*amendment*.json')):raise ValueError('No scope or budget amendment authorized')
...
def transport_ledger(root, attempts, maximum, strict=False):
    ...
    prod=root/'production/encounter-lab'; failures={}; events={}; ledger=[]; tool_receipts=[]
    def identity(record):
    ...
    for p in sorted((prod/'transport-failures').glob('*.json')):
        ...
        if record.get('schema')=='EncounterLabToolFailure/1':
            tool_receipts.append({'path':rel,'sha256':hashed(p)[1],'record':record});continue
        if record.get('schema') not in (None,'EncounterLabTransportFailure/1'):raise ValueError('Unknown transport failure schema')
        ...
        ledger.append({'path':rel,'sha256':hashed(p)[1],'attempt_id':record['attempt_id'],'started':key[1],'state':'failed-no-artwork-returned','schema_validation':'EncounterLabTransportFailure/1' if record.get('schema') else 'explicit field contract; source has no schema tag'})
...
def plan_versions(root):
    ...
    prod=root/'production/encounter-lab';versions={}
    ...
    for path in paths:
        ...
        if record.get('schema')!='EncounterLabPlan/1' or record.get('experiment_id')!='EN-20260909-01' or (len(record.get('entries',[])),frozenset(e.get('id') for e in record.get('entries',[]))) != (24,frozenset(EXPECTED_IDS)):raise ValueError('Historical plan schema/experiment/IDs differ')
...
def prior_source_records(root, plan, binding):
    ...
    if isinstance(declared,list):
        ...
        declared=declared[0]
    if not declared.get('path','').startswith('production/encounter-lab/previous/') or not declared.get('sha256'):raise ValueError('Exact prior choice export must be locally hash-bound')
    binding(declared['path'],declared['sha256'])
    rows=[]
    for path in sorted((root/'production/encounter-lab/previous').glob('*.json')):
        rel=path.relative_to(root).as_posix();digest=hashed(path)[1];binding(rel,digest);record=json.loads(path.read_text())
        rows.append({'path':rel,'sha256':digest,'schema':record.get('schema') if isinstance(record,dict) else None,'experiment_id':record.get('experiment_id') if isinstance(record,dict) else None,'historical_plan_sha256':record.get('plan_sha256') if isinstance(record,dict) else None,'historical_dataset_sha256':record.get('dataset_sha256') if isinstance(record,dict) else None,'validation':'Exact copied file bytes; embedded prior experiment hashes are historical provenance, not EN job/call plan references.'})
    return rows

def research_board_bindings(root, binding, strict=False):
    board=root/'docs/encounter-lab/reference-comparison';research=root/'research/encounter-lab/reference-comparison'
    if not (board/'data.json').exists():return None
    data_path=board/'data.json';data=json.loads(data_path.read_text())
    if data.get('schema')!='EncounterReferenceComparison/1' or data.get('experiment_id')!='EL-REF-20260909-01':raise ValueError('Comparison board identity differs')
    binding(data_path.relative_to(root).as_posix(),hashed(data_path)[1])
    entries=data.get('entries',[])
    if len(entries)!=8 or {x['id'] for x in entries}!={f'COMP-{i:02}' for i in range(1,9)}:raise ValueError('Expected eight reference comparisons')
    def image(path,digest):
        target=(board/path).resolve()
        if not target.is_relative_to(board/'assets'):raise ValueError('Comparison image leaves asset directory')
        binding(target.relative_to(root).as_posix(),digest)
        return target.relative_to(root).as_posix()
    for row in entries:
        image(row['our_image'],row['our_sha256']);image(row['reference_image'],row['reference_sha256'])
    image(data['screenshot']['path'],data['screenshot']['sha256'])
    prior=data.get('prior_references',[])
    if len(prior)!=16 or {x['id'] for x in prior}!={f'REF-{i:02}' for i in range(1,17)}:raise ValueError('Expected sixteen preserved reference excerpts')
    seen=[]
    for row in prior:
        path=image(row['local_image'],row['sha256']);seen.append({'id':row['id'],'path':path,'sha256':row['sha256'],'historical_source_sha256':row['source_sha256']})
    expected='window.COMPARISON_DATA='+data_path.read_text().strip()+';\nwindow.COMPARISON_HASH='+json.dumps(hashed(data_path)[1])+';\n'
    if (board/'data.js').read_text()!=expected:raise ValueError('Comparison JS/JSON data binding differs')
    freeze_path=research/'final-freeze.json';frozen_count=0
    if freeze_path.exists():
        freeze=json.loads(freeze_path.read_text());binding(freeze_path.relative_to(root).as_posix(),hashed(freeze_path)[1]);files=freeze.get('files',[])
        if not files or len({x['path'] for x in files})!=len(files):raise ValueError('Invalid comparison freeze inventory')
        for item in files:
            if not item['path'].startswith(('docs/encounter-lab/reference-comparison/','research/encounter-lab/reference-comparison/')):raise ValueError('Frozen comparison file outside its namespace')
            binding(item['path'],item['sha256'])
            if (root/item['path']).stat().st_size!=item['bytes']:raise ValueError('Frozen comparison file bytes differ')
        if freeze.get('dataset_sha256')!=hashed(data_path)[1]:raise ValueError('Frozen comparison dataset differs')
        frozen_count=len(files)
    elif strict:raise ValueError('Final comparison freeze manifest missing')
    return {'pass':True,'frozen_files':frozen_count,'dataset_sha256':hashed(data_path)[1],'excerpts':seen,'comparison_rows':8,'full_source_cache_excluded':True,'rebuild_policy':'Preserve comparison sources, excerpts and data byte-for-byte; only main encounter reader is rebuilt.'}
...
def source_bindings(root, strict=False, expected_art=None):
    ...
    root = root.resolve(); prod=root/'production/encounter-lab'
    plan=json.loads((prod/'plan.json').read_text())
    if plan.get('schema')!='EncounterLabPlan/1' or plan.get('experiment_id')!='EN-20260909-01' or len(plan['entries'])!=24 or {e['id'] for e in plan['entries']}!=EXPECTED_IDS:
        raise ValueError('Expected exact authorized24-panel EN plan')
    ...
    def walk(value, job_origin=None):
        if isinstance(value,list):
            ...
            if value.get('evidence_path') and not value.get('evidence_sha256'):
                ...
                if not path.startswith(('research/encounter-lab/','production/encounter-lab/')):raise ValueError('Evidence outside current experiment')
    ...
    research_board=research_board_bindings(root,binding,strict)
    ...
    for e in rows:
        ...
        if e['path']!='production/encounter-lab/candidates/'+e['attempt_id']+'.png': raise ValueError('Unexpected native path')
        callpath=e.get('call_path')
        if callpath!='production/encounter-lab/calls/'+e['attempt_id']+'.json': raise ValueError('Missing exact call record')
        ...
        for ref in call.get('references',[]):
            if ref.get('sha256') in research_hashes or ref['path'].startswith(('docs/encounter-lab/reference-comparison/','research/encounter-lab/reference-comparison/')):raise ValueError('External research imagery must not be a generation input')
            if ref['path'].startswith('research/encounter-lab/controls/'):
    ...
    if (prod/'input-validation-failures').exists():raise ValueError('No changed-input retry exception in EN')
    ...
    return {'pass':True,'complete':not missing and len(selections)==24,'registered_attempts':len(rows),'phase_counts':phases,'selected_count':len(selections),'missing_primaries':missing,'checked_bindings':checked,'plan_sha256':plan_sha,'preserved_plan_versions':versions,'prior_source_records':prior_records,'research_board':research_board,'call_plan_bindings':call_plans,'budget':budget_record,'evidence_without_call_hash':list(evidence_observations.values()),'archived_unrun_job_prompts':archived_jobs,'invocation_ledger':invocations}

def gather(root, expected_art=None, strict=False):
    root=root.resolve(); binding_report=source_bindings(root,strict,expected_art)
    helper=root/'research/encounter-lab/asset-bundle'; sources={}; excluded=[]
...
def write_archive(path, manifest, sources):
    ...
    return dict(schema='EncounterLabPortableReceipt/1',archive_bytes=size,archive_sha256=sha,manifest_sha256=hashlib.sha256(encoded(manifest)).hexdigest(),file_count=len(manifest['files']))
...
def local_links(root):
    root=root.resolve(); docs=root/'docs/encounter-lab'; count=0; external=set()
...
def rebuild_check(target):
    ...
    docs=target/'docs/encounter-lab'; names=['data.json','data.js']
    if (docs/'reference-comparison/data.json').exists():names+=['reference-comparison/data.json','reference-comparison/data.js']
    before={n:hashed(docs/n) for n in names}
    binding=source_bindings(target,strict=True,expected_art=len(json.loads((target/'production/encounter-lab/candidates.json').read_text())['candidates']))
    run=subprocess.run([sys.executable,str(target/'research/encounter-lab/reader/build_reader.py'),'--require-complete'],cwd=target,env={**os.environ,'PYTHONDONTWRITEBYTECODE':'1'},text=True,capture_output=True,check=True)
...
def create(root,out,count):
    ...
    allowed=root/'research/encounter-lab/asset-bundle/local'
    ...
    archive=out/'encounter-lab-portable.zip'
...
def self_test():
    ...
    for name,body in [('a.txt',b'AAA'),('b.txt',b'BBB')]:
        p=tmp/name;p.write_bytes(body);sources['production/encounter-lab/'+name]=p
    entries=[dict(path=name,bytes=hashed(p)[0],sha256=hashed(p)[1]) for name,p in sorted(sources.items())]
    manifest=dict(schema='EncounterLabPortable/1',experiment_id='EN-20260909-01',files=entries,total_bytes=6);archive=tmp/'good.zip';receipt=write_archive(archive,manifest,sources)
    checks=[]
    def rejects(label,fn,types=(ValueError,FileExistsError)):
        try:fn()
        except types:checks.append(label)
        else:raise AssertionError(label+' accepted')
    target=tmp/'restore';clash=target/'production/encounter-lab/b.txt';clash.parent.mkdir(parents=True);clash.write_bytes(b'BAD')
    ...
    for bad in ['../escape','production/encounter-lab/../escape','other/a.png','production/encounter-lab/AUX.txt','production\\encounter-lab\\a','/absolute']:
    ...
    for name in ('AGENTS.md','START_HERE.cmd','docs/encounter-lab/index.html'):
        safe_path(name)
    checks.append('EN allowed namespace and root instruction paths')
    for bad in ['production/encounter-labx/a.txt','research/world-combat/a.txt','docs/encounter-lab/name.','docs/encounter-lab/x:y']:
    ...
    for label,names,mode in [('duplicate archive',[entries[0]['path'],entries[0]['path']],None),('symlink archive',[e['path'] for e in entries],stat.S_IFLNK|0o777),('unexpected member',[e['path'] for e in entries]+['production/encounter-lab/c.txt'],None)]:
    ...
    web=tmp/'web';docs=web/'docs/encounter-lab';docs.mkdir(parents=True)
    ...
    (docs/'data.json').write_text(json.dumps({'src':'sample%20image.png?native=1#crop','record_path':'docs/encounter-lab/data.json?record=1#x','tool_output_path':'docs/encounter-lab/data.json','prompt_path':'docs/encounter-lab/data.json','source_path':'/historical/unneeded.png','story_path':'arrival'}))
    ...
    for bad in ['production/encounter-lab/.git/config','research/encounter-lab/asset-bundle/local/a.zip']:
        rejects('excluded member '+bad,lambda bad=bad:safe_path(bad))
    budgetroot=tmp/'budget';bp=budgetroot/'production/encounter-lab';bp.mkdir(parents=True)
    base={'budget':dict(PRODUCTION_BUDGET)}
    (bp/'plan.json').write_bytes(encoded(base));records={}
    assert resolve_budget(budgetroot,base,records)['effective_budget']==BASE_BUDGET;checks.append('fixed 24P 6R1 24F1 54-native 56-invocation budget')
    ...
    tr=tmp/'transport';tp=tr/'production/encounter-lab';(tp/'transport-failures').mkdir(parents=True);(tp/'calls').mkdir()
    failrel='production/encounter-lab/transport-failures/NG01-F1-service-call-01.json';fp=tr/failrel
    reference={'path':'production/encounter-lab/candidates/NG01-P.png','sha256':'a'*64}
    failed={'id':'NG01','attempt_id':'NG01-F1','phase':'texture','category':'vista','retry_of':'NG01-P','prompt_path':'production/encounter-lab/prompts/NG01-F1.txt','prompt_sha256':'b'*64,'plan_sha256':'c'*64,'references':[reference],'referenced_image_paths':['/original/'+reference['path']],'started':{'current_time':'2026-09-08 01:00:00 UTC'},'finished':{'current_time':'2026-09-08 01:01:00 UTC'},'status':'failed-no-artwork-returned','error':'HTTP503','returned_artwork':None}
    ...
    tool={'schema':'EncounterLabToolFailure/1','functions_cell_id':'fixture','failure_record_path':failrel,'failure_record_sha256':hashed(fp)[1],'outcome':{'id':'NG01-F1','status':'error','error':'HTTP503'}}
    ...
    result=dict(schema='EncounterLabBundleSecurityTests/1',status='pass',checks=checks,fixture_directory=str(tmp.relative_to(ROOT)),production_archive_created=False,helper_sha256=hashed(HERE/'bundle_assets.py')[1])
