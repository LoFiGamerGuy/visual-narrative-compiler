#!/usr/bin/env python3
"""Read-only EN baseline audit; write one fresh receipt in asset-bundle/local only."""
import argparse
...
ROOT=HERE.parents[2]
BASELINE=ROOT/'research/encounter-lab/protected-initial.json'
ACTIVE_REF='refs/heads/autonomous/encounter-lab-20260909-0430'
REMOTE_REF='refs/remotes/origin/autonomous/encounter-lab-20260909-0430'
...
def audit():
    ...
    if len(baseline['worktrees'])!=22 or len(baseline['source_files'])!=176:raise ValueError('Unexpected baseline coverage')
    paths=[x['path'] for x in baseline['worktrees']]
    if len(set(paths))!=22 or str(ROOT) in paths:raise ValueError('Protected list must contain22 unique prior worktrees and exclude active root')
    if baseline.get('schema')!='EncounterLabProtectedBaseline/1' or baseline.get('status_args')!=STATUS_ARGS:raise ValueError('Baseline schema or recorded status command differs')
    ...
    return {'schema':'EncounterLabPreservationAudit/1','experiment_id':'EN-20260909-01','started_utc':started,'finished_utc':datetime.now(timezone.utc).isoformat(),
        ...
        'limits':['Status digest includes ignored paths exactly as Git reports them; it does not byte-hash ignored/cache file contents or every tracked/untracked file.',
                  'Only 176 saved original source files have content SHA verification. Existing modified files can retain the same Git status while their bytes change.',
