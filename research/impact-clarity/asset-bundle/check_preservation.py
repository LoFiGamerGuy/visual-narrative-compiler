#!/usr/bin/env python3
"""Read-only IC baseline audit; write one fresh receipt in asset-bundle/local only."""
import argparse
...
ROOT=HERE.parents[2]
BASELINE=ROOT/'research/impact-clarity/protected-initial.json'
ACTIVE_REF='refs/heads/autonomous/nightglass-impact-clarity-20260909-0245'
REMOTE_REF='refs/remotes/origin/autonomous/nightglass-impact-clarity-20260909-0245'
...
def audit():
    ...
    if len(baseline['worktrees'])!=21 or len(baseline['source_files'])!=130:raise ValueError('Unexpected baseline coverage')
    paths=[x['path'] for x in baseline['worktrees']]
    if len(set(paths))!=21 or str(ROOT) in paths:raise ValueError('Protected list must contain21 unique prior worktrees and exclude active root')
    if baseline.get('schema')!='ImpactClarityProtectedBaseline/1' or baseline.get('status_args')!=STATUS_ARGS:raise ValueError('Baseline schema or recorded status command differs')
    ...
    return {'schema':'ImpactClarityPreservationAudit/1','experiment_id':'IC-20260909-01','started_utc':started,'finished_utc':datetime.now(timezone.utc).isoformat(),
        ...
        'limits':['Status digest includes ignored paths exactly as Git reports them; it does not byte-hash ignored/cache file contents or every tracked/untracked file.',
                  'Only 130 saved original source files have content SHA verification. Existing modified files can retain the same Git status while their bytes change.',
