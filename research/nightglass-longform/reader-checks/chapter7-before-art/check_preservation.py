"""Check Chapter7 capture source/copy bindings; never claims visual inspection."""
...
def main():
    ...
    assert current['chapters'][:6] == before['chapters']
    ...
    chapter = next(c for c in current['chapters'] if c['number'] == 7)
    assert chapter['total'] == 40
    ...
    if args.prior:
        ...
        prior_chapter = next(c for c in prior['chapters'] if c['number'] == 7)
    ...
    output.write_text(json.dumps({
        ...
        'script_sha256': sha(authority), 'available': len(records), 'total': 40,
        'previous266_and_comparisons_exact': True,
        'all_old_overrides_and_selected_records_exact': True,
        'prior_chapter7_records_exact_count': prior_count,
    ...
    print('Source/copy preservation PASS:', len(records), '/ 40; no visual verdict')
