...
R=Path('/mnt/c/AgentWorkspaces/anime-pipeline-nightglass-longform-20260909-2110');O=R/'research/nightglass-longform/reader-checks/chapter8-opening-reviewed';O.mkdir(exist_ok=True)
...
with sync_playwright() as p:
 ...
 for name,first,last in [('opening-01','N8-01','N8-01')]:
  out=O/name;out.mkdir(exist_ok=True);bounds=page.evaluate('([a,b])=>({start:document.getElementById(a).getBoundingClientRect().top+scrollY,end:document.getElementById(b).getBoundingClientRect().bottom+scrollY})',[first,last]);start=0;end=math.floor(bounds['end']);last_y=max(0,end-844);ys=sorted(set(list(range(start,max(start+1,last_y),780))+[last_y])) if end-start>844 else [last_y];frames=[]
