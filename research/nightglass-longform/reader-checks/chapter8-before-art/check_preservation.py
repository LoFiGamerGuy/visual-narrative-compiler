"""Check Chapter8 capture source/copy bindings; never claims visual inspection."""
...
def main():
    ...
    assert current['chapters'][:7] == before['chapters']
    ...
    chapter = next(c for c in current['chapters'] if c['number'] == 8)
    assert chapter['total'] == 45
    ...
    if args.prior:
        ...
        prior_chapter = next(c for c in prior['chapters'] if c['number'] == 8)
    ...
    output.write_text(json.dumps({
        ...
        'script_sha256': sha(authority), 'available': len(records), 'total': 45,
        'previous306_and_comparisons_exact': True,
        'all_old_overrides_and_selected_records_exact': True,
        'prior_chapter8_records_exact_count': prior_count,
    ...
    print('Source/copy preservation PASS:', len(records), '/ 45; no visual verdict')
