...
P=ROOT/'production/nightglass-refinement'
...
(P/'candidates.json').write_text(json.dumps({'schema':'NightglassRefinementCandidates/1','experiment_id':'NR-20260908-01','candidates':rows},indent=2)+'\n')
selection_path=P/'selected.json'
selected=json.loads(selection_path.read_text()) if selection_path.exists() else {'schema':'NightglassRefinementSelection/1','experiment_id':'NR-20260908-01','selected':{},'owner_approval':None}
for c in rows:
