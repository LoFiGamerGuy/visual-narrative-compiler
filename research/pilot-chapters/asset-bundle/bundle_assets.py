#!/usr/bin/env python3
"""PC-20260909-01 portable share archive: frozen inputs, safe restore, offline rebuild."""
import argparse
import hashlib
import json
import os
...
ROOT = HERE.parents[2]
PREFIXES = ('production/pilot-chapters/', 'research/pilot-chapters/', 'docs/pilot-chapters/', 'docs/texture-refinement-kit/skill/texture-refinement/')
...
COMPOSITION_IDS = {f'{s}{i:02}' for s in ('NG','BP','ST','RC','FL') for i in range(1,17)}
SUPPORT_IDS = {s+'-SHEET' for s in ('NG','BP','ST','RC','FL')}
EXPECTED_IDS = COMPOSITION_IDS | SUPPORT_IDS
BASE_BUDGET = {'primary':80,'support':5,'structural_repairs':10,'texture_passes':40,'maximum_native_attempts':135,'transport_retries':2,'maximum_total':137}
PRODUCTION_BUDGET = {'primary':80,'support':5,'repair':10,'texture':40,'total_native':135,'no_art_retries':2,'total_invocations':137,'direct_paid_spend_usd':0,'max_repair_per_image':1,'max_texture_per_image':1}
...
def validate_entries(manifest):
    if manifest.get('schema') != 'PilotChaptersPortable/1' or manifest.get('experiment_id') != 'PC-20260909-01':
...
def resolve_budget(root, plan, attempts):
    budget_doc=json.loads((root/'production/pilot-chapters/budget.json').read_text())
    if budget_doc.get('schema')!='PilotChaptersBudget/1' or budget_doc.get('experiment_id')!='PC-20260909-01':raise ValueError('Wrong frozen budget identity')
    base=plan.get('budget',budget_doc['budget'])
    for record in (base,budget_doc['budget']):
        if set(record)-set(PRODUCTION_BUDGET)-{'policy'} or any(type(record.get(k)) is not int or record[k]!=v for k,v in PRODUCTION_BUDGET.items()):raise ValueError('Only frozen80P/5support/10R/40F,135-native/137-call budget authorized')
    if plan.get('scope_amendment') or list((root/'production/pilot-chapters').glob('*amendment*.json')):raise ValueError('No scope or budget amendment authorized')
    return {'base_budget':dict(base),'effective_budget':dict(BASE_BUDGET),'amendment':None,'field_normalization':{'repair':'structural_repairs','texture':'texture_passes','total_native':'maximum_native_attempts','no_art_retries':'transport_retries','total_invocations':'maximum_total'}}
...
def withheld_event(record):
    """Recognize a queue guard that explicitly never invoked the image tool."""
    if record.get('status')!='withheld-before-tool-call':
        if record.get('tool_invoked') is False:raise ValueError('Uninvoked request requires explicit withheld status')
        return False
    if record.get('id') not in EXPECTED_IDS or record.get('attempt_id')!=record['id']+'-P':raise ValueError('Unknown withheld primary identity')
    if record.get('tool_invoked') is not False or record.get('returned_artwork','missing') is not None or not isinstance(record.get('reason'),str) or not record['reason'].strip():raise ValueError('Withheld request must explicitly have no invocation or artwork and state a reason')
    if any(record.get(k) for k in ('output_hint','image_url','image_path','native_path','output_path','transport_retry_of')):raise ValueError('Withheld request conflicts with invocation/output evidence')
    return True
...
def transport_ledger(root, attempts, maximum, strict=False):
    ...
    prod=root/'production/pilot-chapters'; failures={}; events={}; ledger=[]; tool_receipts=[]; administrative=[]
    def identity(record):
    ...
    fields=('id','attempt_id','phase','category','retry_of','source_attempt_id','retry_reason','reason','prompt_path','prompt_sha256','plan_sha256','references')
    ...
    for p in sorted((prod/'transport-failures').glob('*.json')):
        ...
        if record.get('schema')=='PilotChaptersToolFailure/1':
            tool_receipts.append({'path':rel,'sha256':hashed(p)[1],'record':record});continue
        if record.get('schema') not in (None,'PilotChaptersTransportFailure/1'):raise ValueError('Unknown transport failure schema')
        ...
        ledger.append({'path':rel,'sha256':hashed(p)[1],'attempt_id':record['attempt_id'],'started':key[1],'state':'failed-no-artwork-returned','schema_validation':'PilotChaptersTransportFailure/1' if record.get('schema') else 'explicit field contract; source has no schema tag'})
    ...
    for p in sorted((prod/'calls').glob('*.json')):
        ...
        if withheld_event(record):
            if p.stem in attempts:raise ValueError('Withheld request cannot bind a registered artwork')
            administrative.append({'path':p.relative_to(root).as_posix(),'sha256':hashed(p)[1],'attempt_id':record['attempt_id'],'state':record['status'],'counted_as_invocation':False})
            continue
        actual_references(record);key=identity(record);retry_binding(record)
        if key in events:
...
def plan_versions(root):
    ...
    for path in paths:
        ...
        if record.get('schema')!='PilotChaptersPlan/1' or record.get('experiment_id')!='PC-20260909-01' or (len(record.get('entries',[])),frozenset(e.get('id') for e in record.get('entries',[]))) != (len(COMPOSITION_IDS),frozenset(COMPOSITION_IDS)) or (len(record.get('support_entries',[]))>5 or not {e.get('id') for e in record.get('support_entries',[])}<=SUPPORT_IDS):raise ValueError('Historical plan schema/experiment/IDs differ')
        ...
        regular(sidecar)
        if sidecar.read_text().strip() not in (digest,digest+'  '+path.name):raise ValueError('Historical/current plan SHA sidecar differs')
        item={'path':path.relative_to(root).as_posix(),'sha256':digest,'sha256_path':sidecar.relative_to(root).as_posix(),'sha256_file_hash':hashed(sidecar)[1],'entry_ids':[e['id'] for e in record['entries']+record['support_entries']],'effective_budget':policy['effective_budget']}
...
def prior_source_records(root, plan, binding):
    ...
    declared=plan.get('prior_choices') or {'path':'production/pilot-chapters/previous/ce-selection.json','sha256':hashed(root/'production/pilot-chapters/previous/ce-selection.json')[1]}
    if isinstance(declared,list):
        if len(declared)!=1:raise ValueError('Expected one exact historical choice export')
        declared=declared[0]
    if not declared.get('path','').startswith('production/pilot-chapters/previous/') or not declared.get('sha256'):raise ValueError('Exact prior choice export must be locally hash-bound')
    binding(declared['path'],declared['sha256'])
    rows=[]
    for path in sorted((root/'production/pilot-chapters/previous').glob('*.json')):
        rel=path.relative_to(root).as_posix();digest=hashed(path)[1];binding(rel,digest);record=json.loads(path.read_text())
        rows.append({'path':rel,'sha256':digest,'schema':record.get('schema') if isinstance(record,dict) else None,'experiment_id':record.get('experiment_id') if isinstance(record,dict) else None,'historical_plan_sha256':record.get('plan_sha256') if isinstance(record,dict) else None,'historical_dataset_sha256':record.get('dataset_sha256') if isinstance(record,dict) else None,'validation':'Exact copied file bytes; embedded prior experiment hashes are historical provenance, not PC job/call plan references.'})
    return rows
...
def source_bindings(root, strict=False, expected_art=None):
    ...
    root = root.resolve(); prod=root/'production/pilot-chapters'
    plan=json.loads((prod/'plan.json').read_text())
    if plan.get('schema')!='PilotChaptersPlan/1' or plan.get('experiment_id')!='PC-20260909-01' or len(plan['entries'])!=len(COMPOSITION_IDS) or {e['id'] for e in plan['entries']}!=COMPOSITION_IDS or (len(plan.get('support_entries',[]))>5 or not {e['id'] for e in plan.get('support_entries',[])}<=SUPPORT_IDS):
        raise ValueError('Expected eighty PC panels and at most five declared sheets')
    expected_ids=COMPOSITION_IDS|{e['id'] for e in plan.get('support_entries',[])}
    rows=json.loads((prod/'candidates.json').read_text())['candidates'] if (prod/'candidates.json').exists() else []
    if strict and not (prod/'candidates.json').exists():raise ValueError('Final candidate registry missing')
    attempts={e['attempt_id']:e for e in rows}
    if len(attempts)!=len(rows): raise ValueError('Duplicate registered attempts')
    phases={'P':0,'R1':0,'F1':0}; checked={}; archived_jobs=[]; evidence_observations={}
    ...
    def walk(value, job_origin=None):
        if isinstance(value,list):
            ...
            if value.get('attempt_id'):edit_provenance(value)
            if value.get('evidence_path') and not value.get('evidence_sha256'):
                path=value['evidence_path'];safe_path(path)
                if not path.startswith(('research/pilot-chapters/','production/pilot-chapters/')):raise ValueError('Evidence outside current experiment')
                regular(root/path);digest=hashed(root/path)[1];binding(path,digest)
                evidence_observations[path]={'path':path,'inspection_sha256':digest,'recorded_sha256':None,'binding_status':'Call recorded path only; current bytes hashed during package inspection, not retroactively at call time'}
            ...
            for prefix in ('prompt','call','evidence','failure_record','tool_output','control_source','svg','png','scope_amendment'):
    ...
    refs=json.loads((prod/'references.json').read_text())['references']
    if len(refs)!=9 or len({r['id'] for r in refs})!=9:raise ValueError('Expected nine distinct original anchor references')
    if plan.get('reference_library',refs)!=refs:raise ValueError('Reference library disagrees with frozen plan')
    walk(plan); walk(refs)
    prior_records=prior_source_records(root,plan,binding)
    options_path=prod/'options.json';option_doc=json.loads(options_path.read_text());options=option_doc['items']
    if option_doc.get('schema')!='PilotChapterOptions/1' or len(options)!=78 or len({o['id'] for o in options})!=78 or sum(o.get('role')=='original-anchor' for o in options)!=9 or sum(o.get('role')=='prior-art-option' for o in options)!=69:raise ValueError('Expected all nine anchors and all sixty-nine exact AR options')
    binding(options_path.relative_to(root).as_posix(),hashed(options_path)[1]);binding('production/pilot-chapters/budget.json',hashed(prod/'budget.json')[1])
    for option in options:
        binding(option['path'],option['sha256'])
        if option.get('source_sha256')!=option['sha256']:raise ValueError('Option copy differs from original source hash')
        for prefix in ('prompt','call'):
            if option.get(prefix+'_path'):binding(option[prefix+'_path'],option[prefix+'_sha256'])
    # Copied calls keep their old experiment and paths as provenance only; they are not current invocations.
    ...
    for e in rows:
        ...
        if e['path']!='production/pilot-chapters/candidates/'+e['attempt_id']+'.png': raise ValueError('Unexpected native path')
        callpath=e.get('call_path')
        if callpath!='production/pilot-chapters/calls/'+e['attempt_id']+'.json': raise ValueError('Missing exact call record')
        ...
        original, reason=edit_provenance(call,e)
        if e['attempt_id'].endswith('-F1'):
            if len(call.get('references',[]))!=1 or call['references'][0]['path']!=attempts.get(original,{}).get('path'):raise ValueError('F1 must reference only its exact source native')
        for ref in call.get('references',[]):
            if not ref['path'].startswith('production/pilot-chapters/'):raise ValueError('Generation references must be task-owned production assets')
    ...
    controls_path=prod/'controls/manifest.json'
    if controls_path.exists():
        control_doc=json.loads(controls_path.read_text());walk(control_doc)
        if len(control_doc['controls'])!=len({c['id'] for c in control_doc['controls']}):raise ValueError('Duplicate control IDs')
        for c in control_doc['controls']:
            if c.get('png_path') and (c.get('path',c['png_path'])!=c['png_path'] or c.get('sha256',c['png_sha256'])!=c['png_sha256']):raise ValueError('Conflicting control PNG aliases')
            binding(c.get('png_path',c.get('path')),c.get('png_sha256',c.get('sha256')));binding(c['svg_path'],c['svg_sha256'])
    ...
    nonfailed_calls={p.stem for p in (prod/'calls').glob('*.json') if json.loads(p.read_text()).get('status')!='failed-no-artwork-returned' and not withheld_event(json.loads(p.read_text()))}
    ...
    if (prod/'input-validation-failures').exists():raise ValueError('No changed-input retry exception in PC')
    ...
    if phases['P']>budget['primary']+budget['support'] or phases['R1']>budget['structural_repairs'] or phases['F1']>budget['texture_passes'] or len(rows)>budget['maximum_native_attempts']:
    ...
    selections=json.loads((prod/'selected.json').read_text())['selected'] if (prod/'selected.json').exists() else {}
    if strict and not (prod/'selected.json').exists():raise ValueError('Final selection registry missing')
    ...
    if strict and (missing or set(selections)!=expected_ids): raise ValueError('Final package needs all composition/support primaries and selections')
    ...
    return {'pass':True,'complete':not missing and len(selections)==len(expected_ids),'registered_attempts':len(rows),'phase_counts':phases,'selected_count':len(selections),'missing_primaries':missing,'checked_bindings':checked,'plan_sha256':plan_sha,'preserved_plan_versions':versions,'prior_source_records':prior_records,'preserved_option_count':len(options),'primary_compositions':sum(x['id'] in COMPOSITION_IDS and x['attempt_id'].endswith('-P') for x in rows),'primary_support_sheets':sum(x['id'] in SUPPORT_IDS and x['attempt_id'].endswith('-P') for x in rows),'call_plan_bindings':call_plans,'budget':budget_record,'evidence_without_call_hash':list(evidence_observations.values()),'archived_unrun_job_prompts':archived_jobs,'invocation_ledger':invocations}

def gather(root, expected_art=None, strict=False):
    root=root.resolve(); binding_report=source_bindings(root,strict,expected_art)
    helper=root/'research/pilot-chapters/asset-bundle'; sources={}; excluded=[]
    ...
    manifest={'schema':'PilotChaptersPortable/1','experiment_id':'PC-20260909-01','files':entries,'total_bytes':sum(e['bytes'] for e in entries),'source_bindings':binding_report,'explicit_external_receipt_or_launcher_mappings':excluded,'exclusions':'Scratch/runtime/caches/venvs/browser profiles/local directories; no old galleries or original tool-return locations. Complete new namespaces plus AGENTS and textual texture skill/guide only.'}
...
def write_archive(path, manifest, sources):
    ...
    return dict(schema='PilotChaptersPortableReceipt/1',archive_bytes=size,archive_sha256=sha,manifest_sha256=hashlib.sha256(encoded(manifest)).hexdigest(),file_count=len(manifest['files']))
...
def local_links(root):
    root=root.resolve(); docs=root/'docs/pilot-chapters'; count=0; external=set()
    ...
    def walk(value,base):
        if isinstance(value,list):
            ...
            for k,v in value.items():
                if k=='image' and 'local_image' in value:continue
                if isinstance(v,str) and (k in ('src','href','download_src','path','image','local_image','our_image','reference_image') or k.endswith('_path') and k not in ('source_path','story_path')):
...
def rebuild_check(target):
    ...
    docs=target/'docs/pilot-chapters'; names=['data.json','data.js']
    before={n:hashed(docs/n) for n in names}
    binding=source_bindings(target,strict=True,expected_art=len(json.loads((target/'production/pilot-chapters/candidates.json').read_text())['candidates']))
    run=subprocess.run([sys.executable,str(target/'research/pilot-chapters/reader/build_reader.py'),'--require-complete'],cwd=target,env={**os.environ,'PYTHONDONTWRITEBYTECODE':'1'},text=True,capture_output=True,check=True)
...
def create(root,out,count):
    ...
    allowed=root/'research/pilot-chapters/asset-bundle/local'
    ...
    archive=out/'pilot-chapters-portable.zip'
...
def self_test():
    ...
    for name,body in [('a.txt',b'AAA'),('b.txt',b'BBB')]:
        p=tmp/name;p.write_bytes(body);sources['production/pilot-chapters/'+name]=p
    entries=[dict(path=name,bytes=hashed(p)[0],sha256=hashed(p)[1]) for name,p in sorted(sources.items())]
    manifest=dict(schema='PilotChaptersPortable/1',experiment_id='PC-20260909-01',files=entries,total_bytes=6);archive=tmp/'good.zip';receipt=write_archive(archive,manifest,sources)
    checks=[]
    def rejects(label,fn,types=(ValueError,FileExistsError)):
        try:fn()
        except types:checks.append(label)
        else:raise AssertionError(label+' accepted')
    target=tmp/'restore';clash=target/'production/pilot-chapters/b.txt';clash.parent.mkdir(parents=True);clash.write_bytes(b'BAD')
    ...
    for bad in ['../escape','production/pilot-chapters/../escape','other/a.png','production/pilot-chapters/AUX.txt','production\\pilot-chapters\\a','/absolute']:
    ...
    for name in ('AGENTS.md','START_HERE.cmd','docs/pilot-chapters/index.html'):
        safe_path(name)
    checks.append('PC allowed namespace and root instruction paths')
    for bad in ['production/pilot-chaptersx/a.txt','research/world-combat/a.txt','docs/pilot-chapters/name.','docs/pilot-chapters/x:y']:
    ...
    for label,names,mode in [('duplicate archive',[entries[0]['path'],entries[0]['path']],None),('symlink archive',[e['path'] for e in entries],stat.S_IFLNK|0o777),('unexpected member',[e['path'] for e in entries]+['production/pilot-chapters/c.txt'],None)]:
    ...
    web=tmp/'web';docs=web/'docs/pilot-chapters';docs.mkdir(parents=True)
    ...
    (docs/'data.json').write_text(json.dumps({'src':'sample%20image.png?native=1#crop','record_path':'docs/pilot-chapters/data.json?record=1#x','tool_output_path':'docs/pilot-chapters/data.json','prompt_path':'docs/pilot-chapters/data.json','source_path':'/historical/unneeded.png','story_path':'arrival'}))
    ...
    for bad in ['production/pilot-chapters/.git/config','research/pilot-chapters/asset-bundle/local/a.zip']:
        rejects('excluded member '+bad,lambda bad=bad:safe_path(bad))
    budgetroot=tmp/'budget';bp=budgetroot/'production/pilot-chapters';bp.mkdir(parents=True)
    base={'budget':dict(PRODUCTION_BUDGET)}
    (bp/'plan.json').write_bytes(encoded(base));(bp/'budget.json').write_bytes(encoded({'schema':'PilotChaptersBudget/1','experiment_id':'PC-20260909-01','budget':dict(PRODUCTION_BUDGET)}));records={}
    assert resolve_budget(budgetroot,base,records)['effective_budget']==BASE_BUDGET;checks.append('fixed80 panels5support10R1/40F1,135-native137-invocation budget')
    ...
    tr=tmp/'transport';tp=tr/'production/pilot-chapters';(tp/'transport-failures').mkdir(parents=True);(tp/'calls').mkdir()
    failrel='production/pilot-chapters/transport-failures/NG01-F1-service-call-01.json';fp=tr/failrel
    reference={'path':'production/pilot-chapters/candidates/NG01-P.png','sha256':'a'*64}
    failed={'id':'NG01','attempt_id':'NG01-F1','phase':'texture','category':'vista','retry_of':'NG01-P','prompt_path':'production/pilot-chapters/prompts/NG01-F1.txt','prompt_sha256':'b'*64,'plan_sha256':'c'*64,'references':[reference],'referenced_image_paths':['/original/'+reference['path']],'started':{'current_time':'2026-09-08 01:00:00 UTC'},'finished':{'current_time':'2026-09-08 01:01:00 UTC'},'status':'failed-no-artwork-returned','error':'HTTP503','returned_artwork':None}
    ...
    cp=tp/'calls/NG01-F1.json';cp.write_bytes(encoded(retry))
    ...
    x=transport_ledger(tr,{'NG01-F1':{}},2,True);assert x['invocation_count']==2 and x['native_artwork_count']==1;checks.append('successful retry remains one native plus failed invocation')
    ...
    tool={'schema':'PilotChaptersToolFailure/1','functions_cell_id':'fixture','failure_record_path':failrel,'failure_record_sha256':hashed(fp)[1],'outcome':{'id':'NG01-F1','status':'error','error':'HTTP503'}}
    ...
    cp.write_bytes(encoded(again));(tp/'transport-failures/NG01-F1-service-call-02.json').write_bytes(encoded(again))
    ...
    result=dict(schema='PilotChaptersBundleSecurityTests/1',status='pass',checks=checks,fixture_directory=str(tmp.relative_to(ROOT)),production_archive_created=False,helper_sha256=hashed(HERE/'bundle_assets.py')[1])
