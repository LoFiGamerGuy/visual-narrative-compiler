#!/usr/bin/env python3
"""Read-only pilot preservation audit; write a fresh receipt in pilot-chapters/local."""
import argparse
...
ROOT=HERE.parents[1]
BASELINE=ROOT/'research/pilot-chapters/protected-initial.json'
ACTIVE_REF='refs/heads/autonomous/pilot-chapters-20260909-1310'
REMOTE_REF='refs/remotes/origin/autonomous/pilot-chapters-20260909-1310'
...
def compare_topology(old,new):
    ...
    aliases=[]
    for current in list(prior):
        if current in old:continue
        for original in old:
            if Path(current).exists() and Path(original).exists() and Path(current).samefile(Path(original)):
                if original in prior:raise ValueError('Duplicate physical worktree in topology')
                prior[original]=prior.pop(current)
                aliases.append({'before':original,'after':current,'physical_samefile':True})
                break
    changes=[{'path':k,'before':old.get(k),'after':prior.get(k)} for k in sorted(set(old)|set(prior)) if old.get(k)!=prior.get(k)]
    return {'pass':not changes,'total_worktrees_before':len(old),'total_worktrees_now':len(new),'changes':changes,'allowed_added_worktree':active,'active_head_exempt':True,'verified_path_aliases':aliases}

def expand_sources(baseline):
    sources=baseline['source_files']
    if len(sources)!=baseline['source_count'] or len(sources)!=1534:raise ValueError('Protected source expansion count differs')
    return sources,[]
...
def audit():
    ...
    if len(baseline['worktrees'])!=24:raise ValueError('Unexpected baseline coverage')
    paths=[x['path'] for x in baseline['worktrees']]
    if len(set(paths))!=24 or str(ROOT) in paths:raise ValueError('Protected list must contain24 unique prior worktrees and exclude active root')
    if baseline.get('schema')!='PilotChaptersProtectedBaseline/1' or baseline.get('status_args')!=STATUS_ARGS:raise ValueError('Baseline schema or recorded status command differs')
    ...
    return {'schema':'PilotChaptersPreservationAudit/1','experiment_id':'PC-20260909-01','started_utc':started,'finished_utc':datetime.now(timezone.utc).isoformat(),
        ...
        'limits':['Status digest includes ignored paths exactly as Git reports them; it does not byte-hash ignored/cache file contents or every tracked/untracked file.',
                  'Only 1534 saved original source files have content SHA verification. Existing modified files can retain the same Git status while their bytes change.',
                  'Baseline retains raw status text and its SHA; this report compares exact raw status hashes and does not itself render a filename diff.',
...
def main():
    ...
    if not output.is_relative_to(local) or output.suffix!='.json':raise ValueError('--output must be a new .json below pilot-chapters/local')
