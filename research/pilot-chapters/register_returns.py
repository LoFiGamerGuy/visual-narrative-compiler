...
R=Path(__file__).resolve().parents[2];P=R/'production/pilot-chapters';sha=lambda p:hashlib.sha256(p.read_bytes()).hexdigest()
rows=[]
...
(P/'candidates.json').write_text(json.dumps(dict(schema='PilotChaptersCandidates/1',experiment_id='PC-20260909-01',candidates=rows),indent=2)+'\n')
sel=P/'selected.json';s=json.loads(sel.read_text()) if sel.exists() else dict(schema='PilotChaptersDisplaySelection/1',experiment_id='PC-20260909-01',selected={},owner_approval=None)
for c in rows:
