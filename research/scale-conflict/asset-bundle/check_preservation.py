#!/usr/bin/env python3
"""Read-only SC baseline audit; write one fresh receipt in asset-bundle/local only."""
import argparse
...
ROOT=HERE.parents[2]
BASELINE=ROOT/'research/scale-conflict/protected-initial.json'
ACTIVE_REF='refs/heads/autonomous/nightglass-scale-conflict-20260908'
REMOTE_REF='refs/remotes/origin/autonomous/nightglass-scale-conflict-20260908'
EXEMPT_REFS={ACTIVE_REF,REMOTE_REF}
# Baseline argv was not saved; use the lead-specified command and require exact
# equality. The first --ignored mode failed16/19; a bounded probe identified
# --ignored=matching as exact on one dirty tree. Require all19 exact hashes.
# A match reproduces output equivalence, not historical argv recovery.
...
def audit():
    ...
    if len(baseline['worktrees'])!=19 or len(baseline['source_files'])!=7:raise ValueError('Unexpected baseline coverage')
    paths=[x['path'] for x in baseline['worktrees']]
    if len(set(paths))!=19 or str(ROOT) in paths:raise ValueError('Protected list must contain19 unique prior worktrees and exclude active root')
    started=datetime.now(timezone.utc).isoformat()
    with ThreadPoolExecutor(max_workers=4) as pool:
        trees=list(pool.map(check_tree,baseline['worktrees']))
        sources=list(pool.map(check_source,baseline['source_files']))
    ...
    # Only the active worktree HEAD may advance. Its path, branch and other flags
    # remain checked; all prior topology/branch/HEAD records must be identical.
    for mapping in (old,new):
        if str(ROOT) not in mapping:raise ValueError('Active worktree absent from topology')
        if 'branch '+ACTIVE_REF not in mapping[str(ROOT)]:raise ValueError('Active worktree branch changed')
        mapping[str(ROOT)]=[line for line in mapping[str(ROOT)] if not line.startswith('HEAD ')]
    changes=[{'path':k,'before':old.get(k),'after':new.get(k)} for k in sorted(set(old)|set(new)) if old.get(k)!=new.get(k)]
    topology={'pass':not changes,'total_worktrees_before':len(old),'total_worktrees_now':len(new),'changes':changes,'active_head_exempt':str(ROOT)}
    return {'schema':'ScaleConflictPreservationAudit/1','experiment_id':'SC-20260908-01','started_utc':started,'finished_utc':datetime.now(timezone.utc).isoformat(),
        'baseline_path':str(BASELINE.relative_to(ROOT)),'baseline_sha256':file_sha(BASELINE),'checker_sha256':file_sha(Path(__file__)),
        'status_contract':{'args':STATUS_ARGS,'normalization':STATUS_NORMALIZATION,'historical_argv_recovered':False,'equivalence_claim':'Exact status-output SHA equivalence only if all19 match; baseline historical argv was not saved. Git-dir/work-tree explicitly normalized for Windows Git pointers.','read_only_controls':['git --no-optional-locks','GIT_OPTIONAL_LOCKS=0','core.fsmonitor=false']},
        'worktrees':trees,'original_source_files':sources,'refs':refs,'worktree_topology':topology,
        'pass':all(x['pass'] for x in trees+sources) and refs['pass'] and topology['pass'],
        'limits':['Status digest includes ignored paths exactly as Git reports them; it does not byte-hash ignored/cache file contents or every tracked/untracked file.',
                  'Only seven saved original source files have content SHA verification. Existing modified files can retain the same Git status while their bytes change.',
                  ...
                  'Only active local branch/ref, its origin tracking ref and active worktree HEAD may advance. Other new/deleted/changed refs or worktrees fail.']}
