...
amendment_path=P/'budget-amendment-01.json';amendment=json.loads(amendment_path.read_text())
assert amendment['base_plan_sha256']==sha(P/'plan.json')
...
for s in specs:
 ...
 prompt=prefix+s['instruction']+'\nUse this supplied actual image as the ONLY edit reference. Return one new native artwork with the same aspect ratio, not a comparison, panel layout, collage, text, diagram, watermark or annotated explanation.\n'
 ...
 jobs.append(dict(id=id,attempt_id=aid,phase=phase,budget_amendment_path=str(amendment_path.relative_to(R)),budget_amendment_sha256=sha(amendment_path),category=entries[id]['category'],retry_of=c['attempt_id'],retry_reason=s['reason'],evidence_path=str(specpath.relative_to(R)),evidence_sha256=sha(specpath),prompt_path=rel,prompt_sha256=sha(out),plan_sha256=sha(P/'plan.json'),references=[dict(path=c['path'],sha256=c['sha256'],role='Sole exact edit source; preserve all content except the specified '+phase+' change')],prompt=prompt))
