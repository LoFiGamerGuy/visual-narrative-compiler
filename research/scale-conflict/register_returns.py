...
(P/'candidates.json').write_text(json.dumps(dict(schema='ScaleConflictCandidates/1',experiment_id='SC-20260908-01',candidates=rows),indent=2)+'\n')
sel=P/'selected.json';s=json.loads(sel.read_text()) if sel.exists() else dict(schema='ScaleConflictDisplaySelection/1',experiment_id='SC-20260908-01',selected={},owner_approval=None)
for c in rows:
