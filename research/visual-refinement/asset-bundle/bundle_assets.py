#!/usr/bin/env python3
"""VR-20260907-01 portable share archive: frozen inputs, safe restore, offline rebuild."""
import argparse
import hashlib
import json
import os
...
ROOT = HERE.parents[2]
PREFIXES = ('production/visual-refinement/', 'research/visual-refinement/', 'docs/research/visual-refinement/')
...
def gather(root, expected_art):
    if type(expected_art) is not int or not 40 <= expected_art <= 50:
        raise ValueError('Expected final native generation count must be40–50')
    records = []
    for filename in ('setup-candidates.json', 'candidates.json', 'sequence-candidates.json'):
        records += json.loads((root/'production/visual-refinement'/filename).read_text())['candidates']
    ...
    if sum(e['attempt_id'].endswith('-P') for e in records) != 40:
        raise ValueError('Expected exactly40 primary generations')
    refs = json.loads((root/'production/visual-refinement/references.json').read_text())
    if len(refs) != 13:
        raise ValueError('Expected13 copied prior references')
    ...
    for prefix in PREFIXES:
        ...
        for directory, folders, files in os.walk(base, followlinks=False):
            ...
            for name in sorted(files):
                ...
                if source.is_relative_to(root/'research/visual-refinement/asset-bundle'):
                    continue
    ...
    for name in ('bundle_assets.py', 'README.md', 'security-check.json', 'integration-failure-v1.json', 'coverage-failure-v2.json', 'coverage-preflight.json', '.gitignore'):
        sources['research/visual-refinement/asset-bundle/'+name] = HERE/name
    sources['START_HERE.cmd'] = HERE/'START_HERE.cmd'
    entries = []
    for name,path in sorted(sources.items()):
        size,sha = hashed(path)
        entries.append(dict(path=name, bytes=size, sha256=sha))
    manifest = dict(schema='VisualRefinementPortable/1', experiment_id='VR-20260907-01', generated_native_count=len(records), primary_count=40, copied_reference_count=13, files=entries, total_bytes=sum(e['bytes'] for e in entries), exclusions='Runtime, scratch, caches, venv, prior namespaces and original tool-return locations. Current copied native assets and gallery copies retain exact paths.')
...
def write_archive(path, manifest, sources):
    ...
    size,sha = hashed(path)
    return dict(schema='VisualRefinementPortableReceipt/1',archive_bytes=size,archive_sha256=sha,manifest_sha256=hashlib.sha256(encoded(manifest)).hexdigest(),file_count=len(manifest['files']))
...
def rebuild_check(target):
    ...
    docs = target/'docs/research/visual-refinement'
    names = ['comparison-data.json','comparison-data.js','sequence-data.json','sequence-data.js']
    before = {name:hashed(docs/name) for name in names}
    run = subprocess.run([sys.executable,str(target/'research/visual-refinement/reader/build_reader.py'),'--require-comparison','--require-sequence'],cwd=target,env={**os.environ,'PYTHONDONTWRITEBYTECODE':'1'},text=True,capture_output=True,check=True)
...
def create(root, out, count):
    ...
    archive = out/'visual-refinement-portable.zip'
...
def self_test():
    ...
    for name,body in [('a.txt',b'AAA'),('b.txt',b'BBB')]:
        p=tmp/name;p.write_bytes(body);sources['production/visual-refinement/'+name]=p
    ...
    target=tmp/'restore';clash=target/'production/visual-refinement/b.txt';clash.parent.mkdir(parents=True);clash.write_bytes(b'BAD')
    ...
    for bad in ['../escape','production/visual-refinement/../escape','other/a.png','production/visual-refinement/AUX.txt','production\\visual-refinement\\a','/absolute']:
    ...
    for label,names,mode in [('duplicate archive',[entries[0]['path'],entries[0]['path']],None),('symlink archive',[e['path'] for e in entries],stat.S_IFLNK|0o777),('unexpected member',[e['path'] for e in entries]+['production/visual-refinement/c.txt'],None)]:
...
def main():
    ...
    parser.add_argument('--out',type=Path,default=HERE/'local/final-v3')
