#!/usr/bin/env python3
"""WC-20260908-01 portable share archive: frozen inputs, safe restore, offline rebuild."""
import argparse
import hashlib
import json
import os
...
ROOT = HERE.parents[2]
PREFIXES = ('production/world-combat/', 'research/world-combat/', 'docs/world-combat/')
...
def gather(root, expected_art):
    if type(expected_art) is not int or not 24 <= expected_art <= 30:
        raise ValueError('Expected final native generation count must be24–30')
    records = []
    for filename in ('candidates.json',):
        records += json.loads((root/'production/world-combat'/filename).read_text())['candidates']
    ...
    if sum(e['attempt_id'].endswith('-P') for e in records) != 24:
        raise ValueError('Expected exactly24 primary generations')
    refs = json.loads((root/'production/world-combat/references.json').read_text())
    if len(refs) != 9:
        raise ValueError('Expected9 copied prior references')
    for e in records+refs+json.loads((root/'production/world-combat/previous/manifest.json').read_text())['sources']:
    ...
    for prefix in PREFIXES:
        ...
        for directory, folders, files in os.walk(base, followlinks=False):
            ...
            for name in sorted(files):
                ...
                if source.is_relative_to(root/'research/world-combat/asset-bundle'):
                    continue
    ...
    for name in ('bundle_assets.py', 'README.md', 'coverage-preflight.json', '.gitignore'):
        sources['research/world-combat/asset-bundle/'+name] = HERE/name
    sources['START_HERE.cmd'] = HERE/'START_HERE.cmd'
    entries = []
    for name,path in sorted(sources.items()):
        size,sha = hashed(path)
        entries.append(dict(path=name, bytes=size, sha256=sha))
    manifest = dict(schema='WorldCombatPortable/1', experiment_id='WC-20260908-01', generated_native_count=len(records), primary_count=24, copied_reference_count=9, files=entries, total_bytes=sum(e['bytes'] for e in entries), exclusions='Runtime, scratch, caches, venv, prior namespaces and original tool-return locations. Current copied native assets and gallery copies retain exact paths.')
...
def write_archive(path, manifest, sources):
    ...
    size,sha = hashed(path)
    return dict(schema='WorldCombatPortableReceipt/1',archive_bytes=size,archive_sha256=sha,manifest_sha256=hashlib.sha256(encoded(manifest)).hexdigest(),file_count=len(manifest['files']))
...
def rebuild_check(target):
    ...
    docs = target/'docs/world-combat'
    names = ['data.json','data.js']
    before = {name:hashed(docs/name) for name in names}
    run = subprocess.run([sys.executable,str(target/'research/world-combat/reader/build_reader.py'),'--require-complete'],cwd=target,env={**os.environ,'PYTHONDONTWRITEBYTECODE':'1'},text=True,capture_output=True,check=True)
...
def create(root, out, count):
    ...
    archive = out/'world-combat-portable.zip'
...
def self_test():
    ...
    for name,body in [('a.txt',b'AAA'),('b.txt',b'BBB')]:
        p=tmp/name;p.write_bytes(body);sources['production/world-combat/'+name]=p
    ...
    target=tmp/'restore';clash=target/'production/world-combat/b.txt';clash.parent.mkdir(parents=True);clash.write_bytes(b'BAD')
    ...
    for bad in ['../escape','production/world-combat/../escape','other/a.png','production/world-combat/AUX.txt','production\\world-combat\\a','/absolute']:
    ...
    for label,names,mode in [('duplicate archive',[entries[0]['path'],entries[0]['path']],None),('symlink archive',[e['path'] for e in entries],stat.S_IFLNK|0o777),('unexpected member',[e['path'] for e in entries]+['production/world-combat/c.txt'],None)]:
