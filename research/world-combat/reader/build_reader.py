"""Build the isolated world-combat gallery. Source plans, selections and art are read-only."""
...
SOURCE = ROOT / 'production/world-combat'
OUT = ROOT / 'docs/world-combat'
STYLES = ['01', '02', '05', '06', '13', '15', '17', '18', '19']
CATEGORY_IDS = {'scene': [f'S{i:02}' for i in range(1,7)], 'character': [f'C{i:02}' for i in range(1,7)], 'equipment': [f'G{i:02}' for i in range(1,5)], 'wildlife': ['W01','W02'], 'monster': ['M01','M02'], 'ability': [f'A{i:02}' for i in range(1,5)]}
DIMENSIONS = {'scene':['overall','drawing','character','setting','creature','action'], 'character':['overall','drawing','character','weapon','power'], 'equipment':['overall','drawing','design','function'], 'wildlife':['overall','drawing','creature','setting'], 'monster':['overall','drawing','creature','threat'], 'ability':['overall','drawing','power','action']}
LABELS = {'scene':'Full scenes','character':'Characters','equipment':'Equipment','wildlife':'Wildlife','monster':'Monsters','ability':'Abilities'}
...
def asset(value, board=False):
    ...
    if not path.is_relative_to(SOURCE.resolve()):
        raise ValueError('Image path leaves the world-combat namespace')
    raw = path.read_bytes()
...
def previous():
    ...
    old = read(data_path)
    validation = read(ROOT / 'research/world-combat/previous-validation.json')
    ...
    return {'schema':'WorldCombatPreviousView/1','experiment_id':selection['experiment_id'],
...
def build(require_complete=False):
    plan_path = SOURCE / 'world-combat-plan.json'
    plan = read(plan_path)
    if not plan or plan.get('schema') != 'WorldCombatPlan/1':
        raise ValueError('A frozen WorldCombatPlan/1 is required')
    ...
    if len(entries) != 24 or {e['id'] for e in entries} != expected:
        raise ValueError('Exactly24 frozen world-combat IDs are required')
    for e in entries:
    ...
    if notes:
        if notes.get('schema') != 'WorldCombatReviewNotes/1' or notes.get('experiment_id') != plan['experiment_id']:
    ...
    if require_complete and len(selected) != 24:
        raise ValueError('Final reader requires all24 actual selected world-combat images')
    ...
    data = {'schema': 'WorldCombatReader/1', **identity,
            'dataset_sha256': hashlib.sha256(json.dumps(identity, sort_keys=True, separators=(',', ':')).encode()).hexdigest(),
            'styles': plan['styles'], 'categories':[{'id':k,'title':LABELS[k],'dimensions':v} for k,v in DIMENSIONS.items()], 'entries':cards, 'references':references, 'previous':previous(),
            'available_count': len(selected), 'total_count': 24, 'owner_approval': None,
            'review_notes_sha256': sha(notes_path) if notes else None,
            'report_links': [{'label': label, 'path': f'../../research/world-combat/{name}'}
                             for name, label in [('START_HERE.md', 'Study details'), ('RESULTS.md', 'Results & observations')]
                             if (ROOT / f'research/world-combat/{name}').exists()]}
    OUT.mkdir(parents=True, exist_ok=True)
    text = json.dumps(data, indent=2, ensure_ascii=False)
    (OUT / 'data.json').write_text(text + '\n')
    (OUT / 'data.js').write_text('window.WORLD_COMBAT_DATA = ' + text + ';\n')
    return data
