...
plan=read(P/'world-combat-plan.json');ids={e['id'] for e in plan['entries']};assert len(ids)==24
assert collections.Counter(e['category'] for e in plan['entries'])=={'scene':6,'character':6,'equipment':4,'wildlife':2,'monster':2,'ability':4}
...
rejected=[read(p) for p in (P/'rejected-calls').glob('*.json')]
assert len(rejected)==1
for c in rejected:
 assert c['status']=='rejected-before-generation' and c['native_image_returned'] is False and c['primary_generations_consumed']==0
 assert c['attempt_id']=='S06-P' and len(c['references'])==6 and not c.get('output_hint')
...
validated=read(R/'research/world-combat/previous-validation.json');assert validated['pass'] and sha(P/'previous/selection.json')==validated['export_sha256']
...
data=read(R/'docs/world-combat/data.json');assert data['available_count']==24 and data['owner_approval'] is None and data.get('canon') is None
for e in data['entries']:
 c=e['candidate'];assert c['attempt_id']==selected[e['id']] and c['sha256']==by[c['attempt_id']]['sha256']
 assert (R/'docs/world-combat'/c['src']).resolve()==(R/c['path']).resolve()
assert data['plan_sha256']==sha(P/'world-combat-plan.json')
report={'schema':'WorldCombatDeliveryVerification/1','pass':True,'artworks':24,'primary_generations':24,'repairs':len(repairs),'native_attempts':len(rows),'pre_generation_rejections':len(rejected),'references':9,'previous_native_sources':18,'previous_choices_preserved':True,'selected_repairs':sum(a.endswith('-R1') for a in selected.values()),'original_tool_bytes_match':True,'direct_paid_spend_usd':0,'owner_approval':None,'dataset_sha256':data['dataset_sha256']}
(R/'research/world-combat/delivery-verification.json').write_text(json.dumps(report,indent=2)+'\n');print(json.dumps(report))
