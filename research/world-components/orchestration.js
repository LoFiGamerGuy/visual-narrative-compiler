...
async function run(jobs) {
 const root=load("worldRoot"),q=s=>"'"+s.replace(/'/g,"'\\''")+"'";
 ...
 async function save(data,initial=false) {const code="from pathlib import Path;p=Path("+JSON.stringify(root+"/production/world-components/calls/"+j.attempt_id+".json")+");"+(initial?"assert not p.exists();":"")+"p.write_text("+JSON.stringify(JSON.stringify(data,null,2)+"\n")+")";
 ...
 const receipt={...rec,finished:await tools.clock__curr_time({}),output_hint:result.output_hint};store("wcResult:"+j.attempt_id,receipt);await save(receipt);
