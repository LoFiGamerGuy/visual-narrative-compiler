"""Build the instrumented S11 matched-reference-ablation comparison."""
...
OUT = ROOT / "experiments/review-packets/ch05-s11-flat-gouache-reference-ablation-r1/review"
NATIVE = OUT / "s11-reference-ablation-three-column-native-r1.png"
PHONE = OUT / "s11-reference-ablation-three-column-390px-r1.png"
...
def find_s11(document: dict[str, Any]) -> dict[str, Any]:
    ...
    if len(rows) != 1:
        raise ValueError("execution manifest must contain exactly one S11 record")
    return rows[0]
...
def draw_comparison(images: dict[str, Image.Image], path: Path, phone: bool) -> None:
    ...
    if phone:
        for key, image in images.items():
            height = max(1, round(image.height * 390 / image.width))
            prepared[key] = image.resize((390, height), Image.Resampling.LANCZOS)
        column_width = 390
    else:
    ...
    draw.text((margin, 12), "CH05 S11 REFERENCE-ABLATION COMPARISON", fill="#f4f1e8", font=font(26, True))
    draw.text(
        (margin, 48),
...
def main() -> int:
    ...
    if prompt["comparison_contract"]["changed_prompt_line_indexes_zero_based"] != [3]:
        raise ValueError("matched ablation must change exactly line 3")
    if prompt["comparison_contract"]["unchanged_prompt_line_count"] != 15:
        raise ValueError("matched ablation must preserve 15 prompt lines")
    if ablation["input_references"] != [] or text["input_references"] != [] or len(flat["input_references"]) != 2:
        raise ValueError("reference counts differ from the comparison contract")
    ...
    if changed != [3]:
        raise ValueError("source and ablation prompts must differ at exactly one instruction line")
    ...
    if (output["width"], output["height"], output["bytes"]) != (852, 1846, 2376174):
        raise ValueError("ablation output dimensions or byte count mismatch")

    execution = {
        "record_type": "CH05S11MatchedReferenceAblationExecution",
        "schema_version": "1.0",
        "record_id": "ng-ch05-s11-flat-gouache-reference-ablation-execution-r1",
        ...
        "comparison_contract": prompt["comparison_contract"],
        "record": {
            ...
            "execution": {
                ...
                "observed_tool_wall_seconds": 48.2,
                "model": None,
                "provider": None,
                "endpoint": None,
    ...
    evidence = {
        "record_type": "CH05S11MatchedReferenceAblationComparison",
        "schema_version": "1.0",
        "record_id": "ng-ch05-s11-flat-gouache-reference-ablation-comparison-r1",
        ...
        "comparison_design": {
            ...
            "matched_pair_unchanged_prompt_line_count": 15,
        ...
        "review_questions": [
            "Does removing reference pixels change Soren/Sigrid hair, wardrobe, face, or role binding?",
            "Does the matched no-reference result preserve the smoke-realization-run causal sequence?",
            "Does the stricter reduced-palette instruction reduce density without losing urgency or phone readability?",
        ],
        "human_review_state": "PENDING",
        "human_review_minutes": None,
