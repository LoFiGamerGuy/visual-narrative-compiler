"""Build deterministic eight-chapter visual-progression review artifacts."""
...
OUT_DIR = ROOT / "experiments/review-packets/ch06-ch13-progression-review-r1"
MANIFEST = ROOT / "production/comic/run-manifests/ch06-ch13-progression-review-r1.json"
CHAPTERS = ("ch06", "ch07", "ch08", "ch09", "ch10", "ch11", "ch12", "ch13")
...
def main() -> int:
    ...
    contact_out = OUT_DIR / "ch06-ch13-progression-contact-sheet-r1.png"
    sequence_out = OUT_DIR / "ch06-ch13-sequence-progression-r1.png"
    sampler_out = OUT_DIR / "ch06-ch13-phone-progression-sampler-r1.png"
    ...
    stack([sample_row(chapter, packets[chapter]) for chapter in CHAPTERS], 1200, 12, "#11151b").save(
        sampler_out, format="PNG", compress_level=9
    )

    artifacts = []
    for kind, path in (
        ("eight_chapter_contact_sheet", contact_out),
        ("eight_chapter_sequence_progression", sequence_out),
        ("eight_chapter_phone_sampler", sampler_out),
    ):
    ...
    manifest = {
        ...
        "schema_version": "1.0",
        "record_id": "ng-ch06-ch13-progression-review-r1",
        ...
        "limitations": [
            ...
            "All documented warnings and failures remain visible and attributable to their source chapters.",
            "Secondary-adult identity and irreversible garment state remain the leading cross-sequence limitations.",
    ...
    print(json.dumps({"artifacts": 3, "chapters": 8, "panels": 320, "sampled": 72, "triage": triage_total}, sort_keys=True))
    return 0
