"""Build deterministic CH08 RenderRecords, crops, and chapter review artifacts."""
...
PLANS = ROOT / "production/comic/ch08-sc01-panel-plans-r1.json"
PROMPTS = ROOT / "production/comic/run-manifests/ch08-ch09-default-house-route-prompt-manifest-r1.json"
PACKET_DIR = ROOT / "experiments/review-packets/ch08-default-house-route-r1"
SOURCE_DIR = PACKET_DIR / "source"
EXECUTION = ROOT / "production/comic/run-manifests/ch08-default-house-route-execution-r1.json"
PACKET = ROOT / "production/comic/run-manifests/ch08-default-house-route-review-packet-r1.json"
SOURCE_FILES = {
    "ng-ch08-s01-road-kit": "ng-ch08-s01-road-kit.png",
    "ng-ch08-s02-warden-cairns": "ng-ch08-s02-warden-cairns.png",
    "ng-ch08-s03-windthrow": "ng-ch08-s03-windthrow.png",
    "ng-ch08-s04-hollow-stag": "ng-ch08-s04-hollow-stag.png",
    "ng-ch08-s05-root-bridge": "ng-ch08-s05-root-bridge.png",
    "ng-ch08-s06-bound-antler": "ng-ch08-s06-bound-antler.png",
    "ng-ch08-s07-mark-not-enemy": "ng-ch08-s07-mark-not-enemy.png",
    "ng-ch08-s08-black-weir": "ng-ch08-s08-black-weir.png",
}
ELAPSED_SECONDS = {
    "ng-ch08-s01-road-kit": 101.174,
    "ng-ch08-s02-warden-cairns": 216.343,
    "ng-ch08-s03-windthrow": 323.869,
    "ng-ch08-s04-hollow-stag": 430.624,
    "ng-ch08-s05-root-bridge": 106.177,
    "ng-ch08-s06-bound-antler": 212.282,
    "ng-ch08-s07-mark-not-enemy": 330.564,
    "ng-ch08-s08-black-weir": 437.107,
}


def source_boxes(sequence_id: str, image: Image.Image) -> tuple[list[list[int]], dict[str, Any]]:
    if sequence_id == "ng-ch08-s05-root-bridge":
        boxes = [
            [4, 4, 521, 817],
            [524, 4, 844, 817],
            [849, 4, 1172, 817],
        ...
        return boxes, {"method": "HASH_BOUND_MANUAL_RIGHT_EDGE_BOXES", "boxes": boxes}
...
def triage(panel_id: str) -> tuple[str, list[str], str]:
    if panel_id == "ng-ch08-sc01-p028":
        return (
            "WARN",
            ["UNREQUESTED_CAUSAL_ARROW_SYMBOL"],
            "The polehook action is readable, but the generator added a curved instructional arrow near the hook.",
        )
    return (
        "PASS",
        [],
...
def main() -> int:
    ...
    requests = [row for row in prompt_doc["requests"] if row["chapter"] == "CH08"]
    if len(requests) != 8 or sum(len(row["panel_ids"]) for row in requests) != 40:
        raise ValueError("CH08 must have eight preflighted five-panel sequences")
    ...
    if [row["panel_id"] for row in candidates] != [row["panel_id"] for row in plans]:
        raise ValueError("candidate order differs from the 40 ordered CH08 plans")

    contact_cells = [
        labeled_canvas(panel_images[row["panel_id"]], f"P{row['display_order']:03d} · {row['scale_role']}", 238)
        for row in plans
    ]
    contact_path = PACKET_DIR / "ch08-contact-sheet-r1.png"
    ...
    sequence_path = PACKET_DIR / "ch08-sequence-contact-sheet-r1.png"
    stack(sequence_rows, 1200, 10, "#11151b").save(sequence_path, format="PNG", compress_level=9)
    reading_path = PACKET_DIR / "ch08-complete-reading-draft-r1.png"
    ...
    phone_path = PACKET_DIR / "ch08-phone-preview-r1.png"
    ...
    overlay_path = PACKET_DIR / "ch08-lettering-safe-zone-overlay-r1.png"
    ...
    execution_doc = {
        ...
        "schema_version": "1.0",
        "record_id": "ng-ch08-default-house-route-execution-r1",
        ...
        "summary": {
            ...
            "parallel_group_wall_seconds": [430.624, 437.107],
    ...
    packet_doc = {
        ...
        "schema_version": "1.0",
        "record_id": "ng-ch08-default-house-route-review-packet-r1",
        ...
        "limitations": [
            "P028 warns because the generator added a white instructional arrow beside the polehook.",
            "Hollow Stag, evolved clothing, and recovered wire are text-defined because new outputs cannot be re-uploaded as references.",
            "The one-row/no-number and nonverbal-Ledger controls eliminated the exact CH07 grid-label and rendered-prose failures in this chapter.",
