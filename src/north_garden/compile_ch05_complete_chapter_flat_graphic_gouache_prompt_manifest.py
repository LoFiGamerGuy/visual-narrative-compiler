"""Compile a gated, complete CH05 flat graphic gouache lower-density style arm."""
...
OUTPUT = ROOT / "production/comic/run-manifests/ch05-complete-chapter-flat-graphic-gouache-prompt-manifest-r1.json"
REFERENCES = {
...
STYLE = (
    "Style/medium: flat graphic gouache fantasy-adventure webcomic; organize each panel as 4–6 broad poster-like value/color "
    "masses, with crisp contour hierarchy, matte gouache color blocks, strong negative space, and simplified silhouette "
    "backgrounds. Apply minimal localized texture only to the active clue or physical contact. Preserve expressive mature-adult "
    "faces, readable hands and props, causal action, and decisive silhouettes at 390-pixel phone width. No cloth weave, pore detail, "
    "pebble-by-pebble terrain, crosshatching, photorealism, cinematic depth-of-field, grayscale wash, or text of any kind: no "
    "embedded words, speech balloons, captions, labels, panel numbers, sound effects, logos, signatures, or watermark."
)
DENSITY_RULE = (
    "Flat-gouache density cadence: action and reveal anchors may use 5–6 broad masses plus one localized clue/contact texture; "
    "dialogue, deduction, travel, transition, and object-insert beats should use 4–5 broad masses and more negative space. Keep "
    "physical weight, mud, water, smoke, twine, leverage, and footfalls legible through shape, overlap, and pose—not surface noise."
)
INVARIANT = (
    "Flat-gouache arm invariant: exact specified cast; clearly mature fictional adults; stable Soren light-brown/dark-blond hair "
...
def transform(lines: list[str], addenda: list[str]) -> list[str]:
    ...
    for line in lines:
        if line.startswith("Style/medium:"):
            result.extend((STYLE, DENSITY_RULE))
            inserted = True
        else:
            result.append(line)
    if not inserted:
...
def main() -> int:
    ...
    for source in base["sequences"]:
        ...
        sequences.append(
            {
                "sequence_id": f"flat-graphic-gouache-{source['sequence_id']}",
                ...
                "planned_output": (
                    ...
                    f"{source['sequence_id']}-flat-graphic-gouache-r1.png"
                ),
                "execution": None,
                "output": None,
    ...
    if covered != expected or len(sequences) != 11:
        raise ValueError("flat graphic gouache prompt arm must cover 50 plans in canonical order")

    document = {
        "record_type": "CH05CompleteChapterFlatGraphicGouachePromptManifest",
        "schema_version": "1.0",
        "record_id": "ng-ch05-complete-chapter-flat-graphic-gouache-prompts-r1",
        ...
        "style_hypothesis": (
            "A flat graphic gouache chapter constrained to 4–6 broad value/color masses, simplified silhouette backgrounds, and "
            "texture only at the active clue/contact will materially lower visual density while preserving phone-readable role "
            "identity, causal action, continuity, and the unchanged cross-panel semantic gates."
        ),
        "selection_context": (
            "This is a complete-chapter lower-density control for measured comparison with r6, alternate graphic, clear-line "
            "watercolor, and premium cel; it is not a preselected replacement or production acceptance."
        ),
        "coverage": {
