"""Compile CH05 production manifest r3 with the targeted P031-P033 clue repair."""
...
BASE = ROOT / "production/comic/run-manifests/ch05-complete-chapter-production-manifest-r2.json"
REPAIRS = ROOT / "production/comic/run-manifests/ch05-complete-chapter-targeted-repairs-r2.json"
ASSEMBLY = ROOT / "production/comic/run-manifests/ch05-complete-chapter-assembly-manifest-r3.json"
BUILD = ROOT / "experiments/review-packets/ch05-complete-chapter-draft-r3/review/build-report.json"
LETTERING = ROOT / "experiments/review-packets/ch05-complete-chapter-draft-r3/lettered/lettering-build-report.json"
CONTINUITY = ROOT / "experiments/review-packets/ch05-complete-chapter-draft-r3/review/continuity-sheet-report.json"
OUTPUT = ROOT / "production/comic/run-manifests/ch05-complete-chapter-production-manifest-r3.json"
...
def main() -> int:
    ...
    if sha256(crop_report_path) != generation["crop_report"]["sha256"]:
        raise ValueError("clue-chain crop report hash mismatch")
    ...
    for repair in repair_doc["repairs"]:
        ...
        panel["input_references"] = [
            {"path": row["path"], "sha256": row["sha256"], "upload_target": "openai_builtin_imagegen"}
            for row in generation["input_references"]
        ]
        ...
        panel["candidate"] = {
            ...
            "timing_scope": "shared one-strip client observation; do not sum this value across P031-P033",
            "service": {
    ...
    manifest["record_id"] = "ng-ch05-complete-chapter-production-manifest-r3"
    manifest["state"] = "COMPLETE_READING_DRAFT_WITH_CLUE_REPAIRS_PENDING_OWNER_REVIEW"
    ...
    manifest["limitations"] = manifest["limitations"] + [
        "P031-P033 share one source generation and one elapsed observation; three panel records must not be counted as three provider calls or three times the elapsed duration.",
        "Agent triage improves P031/P033 to PASS while P032 remains WARN; all remain pending owner review.",
    ]
