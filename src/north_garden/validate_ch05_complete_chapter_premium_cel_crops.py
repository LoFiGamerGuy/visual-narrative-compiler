"""Validate the premium-cel crop manifest and deterministic split report."""
...
MANIFEST = ROOT / "production/comic/run-manifests/ch05-complete-chapter-premium-cel-crops-r1.json"
REPORT = ROOT / "experiments/review-packets/ch05-complete-chapter-premium-cel-r1/panels/premium-cel-panel-split-report-r1.json"
...
def validate(document: dict[str, Any], report: dict[str, Any] | None, verify_files: bool = True) -> list[str]:
    ...
    check(document.get("record_id") == "ng-ch05-complete-chapter-premium-cel-crops-r1", "record_id")
    ...
    check(document.get("output_filename_template") == "p{panel_number:03d}-premium-cel-r1.png", "template")
    check(
    ...
    check([crop.get("panel_id") for crop in crops] == ids, "canonical panel coverage")
    check([crop.get("display_order") for crop in crops] == list(range(1, 51)), "canonical display order")
    check(
        all(crop.get("plan_revision_id") == plan_by_id[crop.get("panel_id")]["plan_revision_id"] for crop in crops if crop.get("panel_id") in plan_by_id),
    ...
    for index, (sequence, prompt, execution) in enumerate(zip(sequences, prompts, executions, strict=False)):
        ...
        check(gutter.get("mode") == "row_dominance", f"{label} unexpected manual override")
        check(
        ...
        check(all(boxes[pos][3] <= boxes[pos + 1][1] for pos in range(len(boxes) - 1)), f"{label} monotonic crops")
        if verify_files:
    ...
    if report is not None:
        ...
        check([row.get("panel_id") for row in report_panels] == ids, "split canonical order")
        for number, (crop, row, output) in enumerate(zip(crops, report_panels, outputs, strict=False), start=1):
            ...
            check(Path(output.get("path", "")).name == f"p{number:03d}-premium-cel-r1.png", f"split filename {number}")
