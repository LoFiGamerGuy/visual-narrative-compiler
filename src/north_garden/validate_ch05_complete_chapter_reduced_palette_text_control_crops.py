"""Validate the reduced-palette text-control crop manifest and split report."""
...
MANIFEST = ROOT / "production/comic/run-manifests/ch05-complete-chapter-reduced-palette-text-control-crops-r1.json"
REPORT = ROOT / "experiments/review-packets/ch05-complete-chapter-reduced-palette-text-control-r1/panels/reduced-palette-text-control-panel-split-report-r1.json"
...
def validate(document: dict[str, Any], report: dict[str, Any] | None, verify_files: bool = True) -> list[str]:
    ...
    check(document.get("record_id") == "ng-ch05-complete-chapter-reduced-palette-text-control-crops-r1", "record_id")
    ...
    check(document.get("comic_panel_plan_source") == {"path": PLANS.relative_to(ROOT).as_posix(), "sha256": sha256(PLANS)}, "ComicPanelPlan source")
    check(document.get("prompt_manifest") == {"path": PROMPTS.relative_to(ROOT).as_posix(), "sha256": sha256(PROMPTS)}, "prompt manifest source")
    check(document.get("execution_manifest") == {"path": EXECUTIONS.relative_to(ROOT).as_posix(), "sha256": sha256(EXECUTIONS)}, "execution manifest source")
    check(document.get("output_filename_template") == "p{panel_number:03d}-reduced-palette-text-control-r1.png", "template")
    check(document.get("summary") == {
        "sequence_sources": 11,
        "planned_crops": 50,
        "complete_plan_coverage": True,
        "deterministic_gutter_detection": True,
        "automatic_gutter_sequences": 8,
        "manual_gutter_overrides": 3,
    }, "summary")
    ...
    check(all(crop.get("plan_revision_id") == plan_by_id[crop.get("panel_id")]["plan_revision_id"] for crop in crops if crop.get("panel_id") in plan_by_id), "plan revision bindings")
    ...
    for index, (sequence, prompt, execution) in enumerate(zip(sequences, prompts, executions, strict=False)):
        ...
        execution_id = sequence.get("execution_sequence_id")
        ...
        check(execution_id == execution.get("sequence_id"), f"{label} execution sequence")
        ...
        expected_mode = "hash_pinned_manual_override" if override else "row_dominance"
        check(gutter.get("mode") == expected_mode, f"{label} gutter detection mode")
        if override:
            observed_overrides.add(execution_id)
            check(source.get("sha256") == override["source_sha256"], f"{label} override source hash")
            check(gutter.get("override_source_sha256") == override["source_sha256"], f"{label} recorded override source hash")
            check(gutter.get("detected_internal_gutter_extents_inclusive") == override["gutters"], f"{label} exact separator extents")
            check(isinstance(gutter.get("override_justification"), str) and "inspection" in gutter["override_justification"], f"{label} override justification")
        else:
            check(gutter.get("override_source_sha256") is None and gutter.get("override_justification") is None, f"{label} automatic metadata")
        check(isinstance(gutter.get("detected_internal_gutter_extents_inclusive"), list) and len(gutter["detected_internal_gutter_extents_inclusive"]) == len(boxes) - 1, f"{label} gutter count")
        check(all(isinstance(box, list) and len(box) == 4 and all(isinstance(value, int) and not isinstance(value, bool) for value in box) and 0 <= box[0] < box[2] <= source.get("width", -1) and 0 <= box[1] < box[3] <= source.get("height", -1) for box in boxes), f"{label} crop bounds")
        check(all(boxes[position][3] <= boxes[position + 1][1] for position in range(len(boxes) - 1)), f"{label} monotonic crops")
        if verify_files:
            path = ROOT / source.get("path", "")
            check(path.is_file() and sha256(path) == source.get("sha256") and path.stat().st_size == source.get("bytes"), f"{label} source file binding")
            if path.is_file():
                with Image.open(path) as image:
                    check([image.width, image.height] == [source.get("width"), source.get("height")], f"{label} source dimensions")
    check(observed_overrides == set(OVERRIDES), "exact manual override set")

    if report is not None:
        check(report.get("record_type") == "CH05SequenceStripSplitReport", "split report type")
        check(report.get("manifest") == {"path": MANIFEST.relative_to(ROOT).as_posix(), "sha256": sha256(MANIFEST)}, "split report manifest binding")
        check(report.get("summary") == {"sequence_sources": 11, "panels_produced": 50, "complete_plan_coverage": True, "source_hashes_verified": 11, "crop_bounds_verified": 50}, "split report summary")
        check(report.get("planning_structure") == "ComicPanelPlan" and report.get("animation_shot_plan") is None and report.get("e_conte") is None, "split report planning boundary")
        ...
        for number, (crop, row, output) in enumerate(zip(crops, report_panels, outputs, strict=False), start=1):
            ...
            check(Path(output.get("path", "")).name == f"p{number:03d}-reduced-palette-text-control-r1.png", f"split filename {number}")
            ...
            if verify_files:
                ...
                if path.is_file():
                    with Image.open(path) as image:
                        check(image.format == "PNG" and [image.width, image.height] == [output.get("width"), output.get("height")], f"split decode {number}")
    return errors
...
def self_test(document: dict[str, Any]) -> tuple[int, int]:
    mutations: list[Callable[[dict[str, Any]], None]] = [
        ...
        lambda value: value["sequences"][2]["gutter_detection"].__setitem__("mode", "row_dominance"),
        lambda value: value["sequences"][2]["gutter_detection"].__setitem__("override_source_sha256", "0" * 64),
        lambda value: value["sequences"][2]["gutter_detection"]["detected_internal_gutter_extents_inclusive"].pop(),
        lambda value: value["sequences"][0].__setitem__("prompt_sha256", "0" * 64),
        lambda value: value["summary"].__setitem__("manual_gutter_overrides", 2),
...
def main() -> int:
    ...
    print(json.dumps({"errors": errors, "report_present": report is not None, "self_test": f"{caught}/{total}" if arguments.self_test else None, "status": "PASS" if not errors else "FAIL"}, sort_keys=True))
