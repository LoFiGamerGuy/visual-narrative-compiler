"""Emit or validate the final commit-pinned CH05 safe-source inventory r3."""
from __future__ import annotations
import argparse,copy,hashlib,json,subprocess,sys
from pathlib import Path
from validate_safe_source_release_manifest_r2 import git,inventory,root_hash,scope_errors
ROOT=Path(__file__).resolve().parents[2]; PRIOR=ROOT/"docs/research/evidence/ch05-overnight-safe-source-parity-r2.json"; CLOSEOUT=ROOT/"docs/research/evidence/ch05-overnight-closeout-bundle-r1.json"; RELEASE=ROOT/"docs/research/evidence/ch05-overnight-integrated-release-gate-r11.json"; OUTPUT=ROOT/"docs/research/evidence/ch05-overnight-safe-source-parity-r3.json"
class E(RuntimeError): pass
def require(v,m):
    if not v: raise E(m)
def sha(p): return hashlib.sha256(p.read_bytes()).hexdigest()
def build(capture):
    entries=inventory(capture); require(not scope_errors(entries),"; ".join(scope_errors(entries))); prior=json.loads(PRIOR.read_text(encoding="utf-8")); closeout=json.loads(CLOSEOUT.read_text(encoding="utf-8")); release=json.loads(RELEASE.read_text(encoding="utf-8")); require(subprocess.run(["git","merge-base","--is-ancestor",prior["captured_commit"],capture],cwd=ROOT).returncode==0,"r2 capture is not ancestor"); require(closeout["state"]=="PASS_OWNER_PENDING" and closeout["summary"]["candidates"]==29,"closeout changed"); require(release["state"]=="PASS" and release["summary"]["effective_command_count"]==74,"release changed")
    return {"record_type":"ComicSafeSourceDeliveryParity","schema_version":"3.0","record_id":"ng-ch05-overnight-safe-source-parity-r3","state":"PINNED_PUSHED_SAFE_SOURCE_FINAL_RELEASE","repository":"https://github.com/LoFiGamerGuy/visual-narrative-compiler","captured_commit":capture,"captured_tree":git("rev-parse",f"{capture}^{{tree}}"),"captured_origin_main_commit":capture,"remote_parity_at_capture":True,"supersedes":{"path":PRIOR.relative_to(ROOT).as_posix(),"sha256":sha(PRIOR),"tracked_paths":prior["summary"]["tracked_paths"]},"inputs":[{"path":CLOSEOUT.relative_to(ROOT).as_posix(),"sha256":sha(CLOSEOUT)},{"path":RELEASE.relative_to(ROOT).as_posix(),"sha256":sha(RELEASE)}],"summary":{"tracked_paths":len(entries),"total_bytes":sum(x["bytes"] for x in entries),"inventory_root_sha256":root_hash(entries),"public_controls":2,"generated_experiment_paths":0,"prohibited_extensions":0,"files_over_10_mib":0,"provider_credentials":0,"generated_candidate_pixels":0,"model_weights_loras_datasets_private_references":0,"unrelated_untracked_items_in_inventory":0},"entries":entries,"explicit_exclusions":[".env and provider credentials","experiments and generated candidate/review pixels/runtime records","model weights, LoRAs, checkpoints, installed runtimes and caches","datasets and private or likeness references","untracked imported workspace assets, launchers, generators and trainers"],"activity":{"provider_calls":0,"uploads":0,"downloads":0,"paid_spend_usd":0,"acceptance_changes":0,"commercial_clearance_changes":0},"animation_shot_plan":None,"e_conte":None,"boundary":"Final release commit-pinned safe source and non-art evidence only. Ignored local review pixels are linked by hash but are not tracked, published, accepted, or commercially cleared."}
def errors(d,e):
    out=[]
...
def main():
    ...
    try:
        if a.emit:
            require(a.capture,"--capture required with --emit"); capture=git("rev-parse",a.capture); require(git("rev-parse","HEAD")==capture,"capture is not HEAD"); require(git("rev-parse","origin/main")==capture,"capture is not pushed origin/main"); expected=build(capture); OUTPUT.write_text(json.dumps(expected,indent=2)+"\n",encoding="utf-8",newline="\n")
        else:
            tracked=json.loads(OUTPUT.read_text(encoding="utf-8")); capture=tracked["captured_commit"]; expected=build(capture); require(tracked==expected,"tracked r3 parity record differs")
        require(subprocess.run([sys.executable,"src/north_garden/validate_tracked_source_scope.py"],cwd=ROOT).returncode==0,"current tracked scope invalid"); require(subprocess.run(["git","merge-base","--is-ancestor",capture,"HEAD"],cwd=ROOT).returncode==0,"capture is not ancestor of HEAD")
        if not a.allow_unpushed_current: require(git("rev-parse","HEAD")==git("rev-parse","origin/main"),"current HEAD is not at origin/main")
        muts=[lambda x:x.update(state="UNSAFE"),lambda x:x.update(captured_commit="0"*40),lambda x:x.update(captured_tree="0"*40),lambda x:x.update(remote_parity_at_capture=False),lambda x:x["supersedes"].update(sha256="0"*64),lambda x:x["inputs"][1].update(sha256="0"*64),lambda x:x["summary"].update(tracked_paths=x["summary"]["tracked_paths"]-1),lambda x:x["summary"].update(total_bytes=0),lambda x:x["summary"].update(inventory_root_sha256="0"*64),lambda x:x["summary"].update(generated_experiment_paths=1),lambda x:x["summary"].update(prohibited_extensions=1),lambda x:x["summary"].update(files_over_10_mib=1),lambda x:x["summary"].update(provider_credentials=1),lambda x:x["entries"].pop(),lambda x:x["activity"].update(uploads=1),lambda x:x.update(animation_shot_plan={}),lambda x:x["explicit_exclusions"].pop()]; rejected=0
