"""Validate append-only CH05 zero-external-cost ledger r27."""
from __future__ import annotations
import argparse,copy,hashlib,json,sys
from pathlib import Path
ROOT=Path(__file__).resolve().parents[2]; PRIOR=ROOT/"docs/research/evidence/ch05-production-cost-ledger-r26.json"; OUTPUT=ROOT/"docs/research/evidence/ch05-production-cost-ledger-r27.json"
NAMES=["ch05_chapter_production_duration_capacity_r1","ch05_chapter_production_operating_playbook_r1","ch05_overnight_delivery_bundle_r2","ch05_owner_review_index_r6_and_link_manifest_r4"]
def sha(p): return hashlib.sha256(p.read_bytes()).hexdigest()
def ent(n): return {"milestone":n,"external_requests":0,"external_uploads":0,"external_cost_usd":"0.000000"}
def build():
    p=json.loads(PRIOR.read_text(encoding="utf-8")); rows=p["local_zero_external_cost_evidence"]+[ent(n) for n in NAMES]; return {"record_type":"ProductionCostLedger","schema_version":"1.26","record_id":"ng-ch05-production-cost-ledger-r27","supersedes":{"record_id":p["record_id"],"path":PRIOR.relative_to(ROOT).as_posix(),"sha256":sha(PRIOR)},"prior_record_rewritten":False,"budget_domain":p["budget_domain"],"policy_id":p["policy_id"],"state":p["state"],"approved_aggregate_cap_usd":None,"committed_actual_cost_usd":"0.000000","held_reservations_usd":"0.000000","available_usd":None,"currency":"USD","entries":[],"local_zero_external_cost_evidence":rows,"revision_summary":{"prior_local_milestones":64,"appended_local_milestones":4,"total_local_milestones":68,"external_requests":0,"external_uploads":0,"external_cost_usd":"0.000000"},"boundary":"Append-only local evidence accounting. CH05 remains disabled/no-cap/$0 paid API/cloud; built-in product cost remains unavailable and G07 is separate."}
def errors(d,e):
    out=[]
    for k,v in e.items():
...
def main():
    ...
    print(f"CH05 cost ledger r27: {len(fail)} failures; 68 zero-cost milestones; 0 requests/uploads/$0; {rejected}/{len(muts)} mutations rejected")
