"""Validate append-only CH05 zero-external-cost ledger r29."""
from __future__ import annotations
import argparse, copy, hashlib, json, sys
from pathlib import Path
ROOT=Path(__file__).resolve().parents[2]; PRIOR=ROOT/"docs/research/evidence/ch05-production-cost-ledger-r28.json"; OUTPUT=ROOT/"docs/research/evidence/ch05-production-cost-ledger-r29.json"
NAMES=["ch05_overnight_integrated_release_gate_r11","ch05_final_safe_source_and_remote_parity_r1","ch05_overnight_closeout_bundle_r2","ch05_final_model_license_provenance_audit_r1","ch05_owner_response_schema_and_validator_r1","ch05_pilot_root_review_timer_contract_r1","ch05_final_review_session_starter_r1","ch05_owner_ingestion_preflight_contract_r1","ch05_owner_review_index_r8_and_link_manifest_r6"]
def sha(path): return hashlib.sha256(path.read_bytes()).hexdigest()
def entry(name): return {"milestone":name,"external_requests":0,"external_uploads":0,"external_cost_usd":"0.000000"}
def build():
    prior=json.loads(PRIOR.read_text(encoding="utf-8")); rows=prior["local_zero_external_cost_evidence"]+[entry(name) for name in NAMES]; return {"record_type":"ProductionCostLedger","schema_version":"1.28","record_id":"ng-ch05-production-cost-ledger-r29","supersedes":{"record_id":prior["record_id"],"path":PRIOR.relative_to(ROOT).as_posix(),"sha256":sha(PRIOR)},"prior_record_rewritten":False,"budget_domain":prior["budget_domain"],"policy_id":prior["policy_id"],"state":prior["state"],"approved_aggregate_cap_usd":None,"committed_actual_cost_usd":"0.000000","held_reservations_usd":"0.000000","available_usd":None,"currency":"USD","entries":[],"local_zero_external_cost_evidence":rows,"revision_summary":{"prior_local_milestones":73,"appended_local_milestones":9,"total_local_milestones":82,"external_requests":0,"external_uploads":0,"external_cost_usd":"0.000000"},"boundary":"Append-only local evidence accounting. CH05 remains disabled/no-cap/$0 paid API/cloud; built-in product cost remains unavailable and G07 is separate."}
...
def main():
    ...
    print(f"CH05 cost ledger r29: {len(failures)} failures; 82 zero-cost milestones; 0 requests/uploads/$0; {rejected}/{len(mutations)} mutations rejected")
