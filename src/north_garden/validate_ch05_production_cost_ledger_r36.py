"""Validate append-only CH05 production cost ledger r36."""
...
LEDGER = ROOT / "docs/research/evidence/ch05-production-cost-ledger-r36.json"
...
def validate(document: dict[str, Any]) -> list[str]:
    ...
    check(
        ...
        and document.get("schema_version") == "1.35"
        and document.get("record_id") == "ng-ch05-production-cost-ledger-r36",
        "identity",
    )
    check(
    ...
    check(
        ...
        == {
            "prior_local_milestones": 117,
            "appended_local_milestones": 5,
            "total_local_milestones": 122,
            "external_requests": 0,
            "external_uploads": 0,
            "external_cost_usd": "0.000000",
        }
...
def self_test(document: dict[str, Any]) -> tuple[int, int]:
    mutations: list[Callable[[dict[str, Any]], None]] = [
        ...
        lambda value: value["local_zero_external_cost_evidence"][-1].__setitem__("external_cost_usd", "1.000000"),
        lambda value: value["revision_summary"].__setitem__("total_local_milestones", 121),
