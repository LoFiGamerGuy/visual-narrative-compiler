"""Validate append-only CH05 production cost ledger r37."""
...
LEDGER = ROOT / "docs/research/evidence/ch05-production-cost-ledger-r37.json"
...
def validate(document: dict[str, Any]) -> list[str]:
    ...
    check(
        ...
        and document.get("schema_version") == "1.36"
        and document.get("record_id") == "ng-ch05-production-cost-ledger-r37",
        "identity",
    )
    check(
    ...
    check(document.get("prior_record_rewritten") is False, "prior rewrite")
    ...
    check(
        ...
        == {
            "prior_local_milestones": 122,
            "appended_local_milestones": 2,
            "total_local_milestones": 124,
            "external_requests": 0,
            "external_uploads": 0,
            "external_cost_usd": "0.000000",
        }
    ...
    check(
        document.get("current_revision_activity")
        == {
            "provider_calls": 0,
            "external_uploads": 0,
            "generation_calls": 0,
    ...
    for key in (
        ...
        check(document.get(key) == prior.get(key), f"prior field:{key}")
    boundary = document.get("boundary", "")
    check(
        "zero provider calls" in boundary
        and "unavailable service metadata" in boundary
        and "no review, acceptance" in boundary,
        "boundary semantics",
    )
    return errors
...
def self_test(document: dict[str, Any]) -> tuple[int, int]:
    mutations: list[Callable[[dict[str, Any]], None]] = [
        ...
        lambda value: value["revision_summary"].__setitem__("total_local_milestones", 123),
        lambda value: value["current_revision_local_evidence_sources"].pop(),
        lambda value: value["current_revision_local_evidence_sources"][0].__setitem__("sha256", "0" * 64),
        lambda value: value["current_revision_activity"].__setitem__("provider_calls", 1),
        lambda value: value["current_revision_activity"].__setitem__("generated_candidates", 1),
