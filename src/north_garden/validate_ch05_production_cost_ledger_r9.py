"""Validate append-only CH05 zero-external-cost ledger r9."""
from __future__ import annotations
import argparse,copy,hashlib,json,sys
from pathlib import Path
ROOT=Path(__file__).resolve().parents[2]; R8=ROOT/"docs/research/evidence/ch05-production-cost-ledger-r8.json"; R9=ROOT/"docs/research/evidence/ch05-production-cost-ledger-r9.json"
NAMES=["safe_source_release_manifest_r2"]
...
def build():
 p=json.loads(R8.read_text(encoding="utf-8")); add=[ent(n) for n in NAMES]; all=p["local_zero_external_cost_evidence"]+add
 return {"record_type":"ProductionCostLedger","schema_version":"1.8","record_id":"ng-ch05-production-cost-ledger-r9","supersedes":{"record_id":p["record_id"],"path":R8.relative_to(ROOT).as_posix(),"sha256":sha(R8)},"prior_record_rewritten":False,"budget_domain":p["budget_domain"],"policy_id":p["policy_id"],"state":p["state"],"approved_aggregate_cap_usd":None,"committed_actual_cost_usd":"0.000000","held_reservations_usd":"0.000000","available_usd":None,"currency":"USD","entries":[],"local_zero_external_cost_evidence":all,"revision_summary":{"prior_local_milestones":len(p["local_zero_external_cost_evidence"]),"appended_local_milestones":len(add),"total_local_milestones":len(all),"external_requests":0,"external_uploads":0,"external_cost_usd":"0.000000"},"boundary":"Append-only local evidence accounting. CH05 remains disabled/no-cap/$0; G07 availability remains separate."}
def muts(e):
...
def main():
 ...
 try:
  ...
  else:req(json.loads(R9.read_text(encoding="utf-8"))==e,"tracked r9 differs")
