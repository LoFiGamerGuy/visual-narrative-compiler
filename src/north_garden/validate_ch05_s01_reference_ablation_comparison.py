"""Validate S01 reference-ablation execution and three-column review evidence."""
...
def find_s01(document: dict[str, Any]) -> dict[str, Any]:
    ...
    if len(rows) != 1:
        raise ValueError("source execution must contain exactly one S01")
    return rows[0]
...
def validate(execution: dict[str, Any], evidence: dict[str, Any]) -> list[str]:
    ...
    check(
        execution.get("record_type") == "CH05S01MatchedReferenceAblationExecution"
        and execution.get("schema_version") == "1.0"
        and execution.get("record_id") == "ng-ch05-s01-flat-gouache-reference-ablation-execution-r1"
    ...
    check(
        contract.get("changed_prompt_line_indexes_zero_based") == [3]
        and contract.get("unchanged_prompt_line_count") == 17
    ...
    check(changed == [3] and len(flat_lines) == 18, "actual one-line prompt difference")
    check(
    ...
    check(
        ...
        and record.get("source_sequence_id") == "s01-opening-departure"
        and record.get("panel_range") == [1, 5]
        and record.get("panel_count") == 5
    ...
    check(
        ...
        and run.get("observed_tool_wall_seconds") == 110.4,
    ...
    check(
        ...
        and output_facts == (OUTPUT_SHA256, 852, 1846, 2369798),
    ...
    check(
        evidence.get("record_type") == "CH05S01MatchedReferenceAblationComparison"
        and evidence.get("schema_version") == "1.0"
        and evidence.get("record_id") == "ng-ch05-s01-flat-gouache-reference-ablation-comparison-r1"
    ...
    check(
        ...
        and design.get("matched_pair_unchanged_prompt_line_count") == 17
    ...
    expected_artifacts = {"native_three_column": (2635, 1978), "phone_390px_three_column": (1246, 977)}
...
def self_test(execution: dict[str, Any], evidence: dict[str, Any]) -> tuple[int, int]:
    mutations: list[tuple[str, Callable[[dict[str, Any]], None]]] = [
        ...
        ("execution", lambda value: value["record"].__setitem__("rights_cleared", True)),
