"""Validate the deterministic CH06-CH09 progression review."""
...
from build_ch06_ch09_progression_review import CHAPTERS, MANIFEST, ROOT, sha256
...
def validate(document: dict[str, Any], *, files: bool) -> list[str]:
    ...
    if len(packets) != 4:
        errors.append("four source packets are required")
    else:
        for row in packets:
    ...
    required = {"four_chapter_contact_sheet", "four_chapter_sequence_progression", "four_chapter_phone_sampler"}
...
def self_test(document: dict[str, Any]) -> tuple[int, int]:
    ...
    for mutate in (
        ...
        lambda d: d["summary"].__setitem__("panel_candidates", 159),
        lambda d: d["summary"].__setitem__("accepted", 1),
        lambda d: d["summary"]["triage"].__setitem__("PASS", 154),
