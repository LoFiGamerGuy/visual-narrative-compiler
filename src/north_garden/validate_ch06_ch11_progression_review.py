"""Validate the deterministic CH06-CH11 progression review."""
...
from build_ch06_ch11_progression_review import CHAPTERS, MANIFEST, ROOT, sha256
...
def validate(document: dict[str, Any], *, files: bool) -> list[str]:
    ...
    if len(packets) != 6:
        errors.append("six source packets are required")
    else:
        for row in packets:
    ...
    expected = {
        ...
        "sequence_sources": 48,
        "triage": {"PASS": 225, "WARN": 5, "FAIL": 10},
        "sampled_panels": 54,
    ...
    required = {"six_chapter_contact_sheet", "six_chapter_sequence_progression", "six_chapter_phone_sampler"}
...
def self_test(document: dict[str, Any]) -> tuple[int, int]:
    ...
    for mutate in (
        ...
        lambda d: d["summary"].__setitem__("panel_candidates", 239),
        lambda d: d["summary"].__setitem__("accepted", 1),
        lambda d: d["summary"]["triage"].__setitem__("PASS", 226),
