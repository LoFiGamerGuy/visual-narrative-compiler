"""Validate the one-route CH08-CH09 built-in ImageGen prompt manifest."""
...
MANIFEST = ROOT / "production/comic/run-manifests/ch08-ch09-default-house-route-prompt-manifest-r1.json"
PLANS = {
...
REQUIRED_PHRASES = (
    ...
    "never a gun, firearm, crossbow, or complex machine",
...
BANNED_GENERATED_COPY = (
    "ROAD TENDED inscription",
    "ROUTE KEEPER inscription",
    "BELOW / ROUTE UNCONFIRMED",
    "WAYFINDER PATH / EARNED BY RETURN",
)
...
def errors(data: dict[str, Any], *, check_files: bool = True) -> list[str]:
    ...
    check(summary.get("reference_uses") == 40 == sum(row.get("reference_count", 0) for row in requests), "reference uses")
