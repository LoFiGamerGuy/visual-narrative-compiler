"""Validate CH08 default-route RenderRecords, crops, timing, and review packet."""
...
from build_ch08_default_route_review import ELAPSED_SECONDS
...
def validate(execution: dict[str, Any], packet: dict[str, Any], *, check_files: bool) -> list[str]:
    ...
    prompts = {row["request_id"]: row for row in load(PROMPTS)["requests"] if row["chapter"] == "CH08"}
    ...
    for index, row in enumerate(records):
        ...
        if prompt is None:
            errors.append(f"{label} is not a CH08 request")
            continue
        ...
        if not str(output.get("path", "")).startswith("experiments/review-packets/ch08-default-house-route-r1/source/"):
    ...
    if reference_uses != 20:
        errors.append(f"authorized reference uses must be 20, got {reference_uses}")
    ...
    if summary.get("parallel_group_wall_seconds") != [430.624, 437.107]:
    ...
    if triage != {"PASS": 39, "WARN": 1, "FAIL": 0}:
        errors.append(f"triage must preserve 39/1/0, got {triage}")
    if len(candidates) == 40 and candidates[27].get("failure_classes") != ["UNREQUESTED_CAUSAL_ARROW_SYMBOL"]:
        errors.append("P028 must preserve the arrow-symbol warning")
...
def self_test(execution: dict[str, Any], packet: dict[str, Any]) -> tuple[int, int]:
    ...
    for mutate_e, mutate_p in (
        ...
        (None, lambda d: d["candidates"][27].__setitem__("failure_classes", [])),
        (None, lambda d: d["candidates"][27].__setitem__("agent_triage", "PASS")),
