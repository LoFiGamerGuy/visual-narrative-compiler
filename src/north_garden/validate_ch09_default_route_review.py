"""Validate CH09 default-route RenderRecords, crops, timing, and review packet."""
...
from build_ch09_default_route_review import ELAPSED_SECONDS, RECONSTRUCTED_TIMINGS
...
def validate(execution: dict[str, Any], packet: dict[str, Any], *, check_files: bool) -> list[str]:
    ...
    prompts = {row["request_id"]: row for row in load(PROMPTS)["requests"] if row["chapter"] == "CH09"}
    ...
    for index, row in enumerate(records):
        ...
        if prompt is None:
            errors.append(f"{label} is not a CH09 request")
            continue
        ...
        expected_source = (
            ...
            if request_id in RECONSTRUCTED_TIMINGS
            else "CLIENT_WRAPPER_DATE_NOW_AROUND_IMAGEGEN_CALL"
        )
        if row.get("elapsed_source") != expected_source:
        ...
        if not str(output.get("path", "")).startswith("experiments/review-packets/ch09-default-house-route-r1/source/"):
    ...
    if summary.get("parallel_group_wall_seconds") != [437.501, 417.841]:
    ...
    if len(candidates) == 40 and candidates[36].get("failure_classes") != ["WEAK_FORENSIC_SCRAPE_ARCS"]:
        errors.append("P037 must preserve the weak-forensic-scrape-arcs warning")
...
def self_test(execution: dict[str, Any], packet: dict[str, Any]) -> tuple[int, int]:
    ...
    for mutate_e, mutate_p in (
        ...
        (None, lambda d: d["candidates"][36].__setitem__("failure_classes", [])),
        (None, lambda d: d["candidates"][36].__setitem__("agent_triage", "PASS")),
