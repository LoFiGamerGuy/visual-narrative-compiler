"""Validate CH10 default-route RenderRecords, crops, timing, and review packet."""
...
from build_ch10_default_route_review import ELAPSED_SECONDS
...
def validate(execution: dict[str, Any], packet: dict[str, Any], *, check_files: bool) -> list[str]:
    ...
    prompts = {row["request_id"]: row for row in load(PROMPTS)["requests"] if row["chapter"] == "CH10"}
    ...
    for index, row in enumerate(records):
        ...
        if prompt is None:
            errors.append(f"{label} is not a CH10 request")
            continue
        ...
        if not str(output.get("path", "")).startswith("experiments/review-packets/ch10-default-house-route-r1/source/"):
    ...
    if reference_uses != 23:
        errors.append(f"authorized reference uses must be 23, got {reference_uses}")
    ...
    if summary.get("parallel_group_wall_seconds") != [403.285, 414.710]:
    ...
    if triage != {"PASS": 37, "WARN": 0, "FAIL": 3}:
        errors.append(f"triage must preserve 37/0/3, got {triage}")
    if len(candidates) == 40:
        if candidates[6].get("failure_classes") != ["HALVOR_ROLE_IDENTITY_SUBSTITUTION"]:
            errors.append("P007 must preserve the Halvor identity failure")
        if candidates[8].get("failure_classes") != ["HALVOR_ROLE_IDENTITY_SUBSTITUTION"]:
            errors.append("P009 must preserve the Halvor identity failure")
        if candidates[22].get("failure_classes") != ["PREMATURE_UNREQUESTED_MIREBACK"]:
            errors.append("P023 must preserve the premature Mireback failure")
...
def self_test(execution: dict[str, Any], packet: dict[str, Any]) -> tuple[int, int]:
    ...
    for mutate_e, mutate_p in (
        ...
        (None, lambda d: d["candidates"][6].__setitem__("failure_classes", [])),
        (None, lambda d: d["candidates"][8].__setitem__("agent_triage", "PASS")),
        (None, lambda d: d["candidates"][22].__setitem__("failure_classes", [])),
