"""Pin and validate the final hardening-release safe-source Git inventory."""
...
DEFAULT_COMMIT = "f1803bddcaea1906b79aa3520e72036ada1b7354"
R3 = ROOT / "docs/research/evidence/safe-source-release-manifest-00498df.json"
OUTPUT = ROOT / "docs/research/evidence/safe-source-release-manifest-f1803bd.json"
...
def build(commit: str) -> dict:
    ...
    prior = json.loads(R3.read_text(encoding="utf-8"))
    require(prior["captured_commit"] == "00498df557e56889ce161095572fcf1d09d95498", "r3 capture changed")
    require(subprocess.run(["git", "merge-base", "--is-ancestor", prior["captured_commit"], full_commit], cwd=ROOT).returncode == 0, "r3 capture is not an ancestor")
    return {
        "record_type": "SafeSourceReleaseManifest",
        "schema_version": "1.3",
        "record_id": "ng-safe-source-release-f1803bd",
...
def main() -> int:
    ...
    try:
        ...
        if args.emit:
            ...
            require(json.loads(OUTPUT.read_text(encoding="utf-8")) == expected, "tracked safe-source r4 differs")
